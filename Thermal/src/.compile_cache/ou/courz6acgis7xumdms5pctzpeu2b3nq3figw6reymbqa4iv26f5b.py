...
# kernel path: ./.compile_cache/yr/cyrbmgytlci6s6fm3hbrjatrwuqw2ojcc3esqx5lnhsryjoo6bcm.py
...
#   x_next => add
# Graph fragment:
#   %arg2_1 : Tensor "f32[6, 4, 32, 32][4096, 1024, 32, 1]cuda:0" = PlaceHolder[target=arg2_1]
#   %arg1_1 : Tensor "f32[6][1]cuda:0" = PlaceHolder[target=arg1_1]
#   %randn : Tensor "f32[6, 4, 32, 32][4096, 1024, 32, 1]cuda:0" = PlaceHolder[target=randn]
#   %view : Tensor "f32[6, 1, 1, 1][1, 1, 1, 1]cuda:0"[num_users=1] = call_function[target=torch.ops.aten.reshape.default](args = (%arg1_1, [-1, 1, 1, 1]), kwargs = {})
...
#   %mul : Tensor "f32[6, 1, 1, 1][1, 1, 1, 1]cuda:0"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%view, %full_default), kwargs = {})
#   %mul_1 : Tensor "f32[6, 4, 32, 32][4096, 1024, 32, 1]cuda:0"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%mul, %randn), kwargs = {})
#   %add : Tensor "f32[6, 4, 32, 32][4096, 1024, 32, 1]cuda:0"[num_users=1] = call_function[target=torch.ops.aten.add.Tensor](args = (%arg2_1, %mul_1), kwargs = {})
#   return %add
...
def partition_0(args):
    arg2_1, arg1_1 = args
    args.clear()
    assert_size_stride(arg2_1, (6, 4, 32, 32), (4096, 1024, 32, 1))
    assert_size_stride(arg1_1, (6, ), (1, ))
    with torch.cuda._DeviceGuard(0):
        torch.cuda.set_device(0)
        # Topologically Sorted Source Nodes: [noise], Original ATen: [aten.randn_like]
        buf0 = torch.ops.aten.randn.default([6, 4, 32, 32], dtype=torch.float32, device=device(type='cuda', index=0), pin_memory=False)
        buf1 = buf0
        assert_size_stride(buf1, (6, 4, 32, 32), (4096, 1024, 32, 1), 'torch.ops.aten.randn.default')
        assert_alignment(buf1, 16, 'torch.ops.aten.randn.default')
        del buf0
        buf2 = buf1; del buf1  # reuse
        # Topologically Sorted Source Nodes: [sigma, wrapped_sqrt, mul, mul_1, x_next], Original ATen: [aten.view, aten.sqrt, aten.mul, aten.add]
        stream0 = get_raw_stream(0)
        triton_poi_fused_add_mul_sqrt_view_0.run(buf2, arg2_1, arg1_1, 24576, stream=stream0)
        del arg1_1
        del arg2_1
    return (buf2, )
...
def benchmark_compiled_module(times=10, repeat=10):
    ...
    arg0_1 = rand_strided((6, 4, 32, 32), (4096, 1024, 32, 1), device='cuda:0', dtype=torch.float32)
    arg1_1 = rand_strided((6, ), (1, ), device='cuda:0', dtype=torch.float32)
    arg2_1 = rand_strided((6, 4, 32, 32), (4096, 1024, 32, 1), device='cuda:0', dtype=torch.float32)
