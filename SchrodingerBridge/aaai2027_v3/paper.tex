...
These rows test whether the main result survives changes in scope, representation, style set, and dataset bias, and whether the learned transport can be reused without full retraining.}
\label{tab:controls}
\scriptsize
\renewcommand{\arraystretch}{0.80}
...
Random-20 extension & all & 0.7434 & 0.2910 \\
\bottomrule
\end{tabular}
\end{table}
...
As shown in Table~\ref{tab:controls}, WD-VF reaches 0.7434 CLIP-S and 0.2910 LPIPS, comfortably retaining the low-damage, correct-direction regime.
The all-pairs transfer CLIP-S is 0.7392 and the identity CLIP-S is 0.8239, so the model still moves away from the source distribution rather than collapsing to it.
This confirms that the Distinct5 result is not an artifact of the five selected styles: the wavelet-decoupled transport generalizes to a conventional, larger vocabulary.
...
The Random-20 experiment in Table~\ref{tab:controls} partially addresses this concern by showing that the same low-damage, correct-direction regime survives on a larger, non-cherry-picked style vocabulary.
