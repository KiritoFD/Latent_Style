...
def _load_vavae(device="cuda", cache_dir=None):
    """Load VAVAE (hustvl/vavae-imagenet256-f16d32-dinov2).

    VAVAE uses LDM's AutoencoderKL with 32 latent channels and 16x downsampling.
    encode() returns DiagonalGaussianDistribution (has .sample()).
    decode(z) returns tensor directly (NOT .sample).
    """
...
def _is_vavae(vae) -> bool:
    """Check if the VAE is a VAVAE model (LDM-style AutoencoderKL)."""
    return hasattr(vae, "model_type") and getattr(vae, "model_type", "") == "vavae"


def _vae_scaling_factor(vae) -> float:
    """Get the scaling factor from any VAE type."""
    if _is_vavae(vae):
        return 1.0  # VAVAE uses no scaling
    cfg = getattr(vae, "config", None)
    if cfg is not None:
        return float(getattr(cfg, "scaling_factor", 0.18215))
    # FLUX AE fallback
    if _is_flux_ae(vae):
        return 0.3611
    return 0.18215
...
def _load_flux_ae(device="cuda", cache_dir=None):
    """Load FLUX.1 AE (16-channel latent, f8, scaling_factor=0.3611)."""
    try:
    ...
    try:
        vae = AutoencoderKL.from_pretrained(
            "black-forest-labs/FLUX.1-schnell", subfolder="ae",
        ...
        vae = AutoencoderKL.from_pretrained(
            "black-forest-labs/FLUX.1-schnell", subfolder="ae", **kwargs,
        ).to(device)
    vae.eval()
    # Tag for detection
    vae._vae_family = "flux"
...
def _is_flux_ae(vae) -> bool:
    """Check if the VAE is a FLUX.1 AE (16-channel latent)."""
    cfg = getattr(vae, "config", None)
    if cfg is not None:
        ch = getattr(cfg, "latent_channels", None)
        if ch is not None and int(ch) == 16:
            return True
    return getattr(vae, "_vae_family", "") == "flux"


def download_vae_with_fallback(model_id, device="cuda", cache_dir=None):
    model_key = str(model_id).strip().lower()
    # VAVAE (f16d32, 32-channel latent) — uses a separate loading path
    if model_key == "vavae":
        return _load_vavae(device=device, cache_dir=cache_dir)
    # FLUX.1 AE (16-channel latent, f8)
    if model_key in {"flux", "flux1", "flux-ae", "flux.1"}:
        return _load_flux_ae(device=device, cache_dir=cache_dir)
...
def load_vae(
    ...
    *,
    ...
    # VAVAE: keep float32, skip diffusers-specific methods
    if _is_vavae(vae):
        logger.info("VAVAE loaded (f16d32, 32-channel, no scaling)")
        return vae
...
def encode_image(vae, image_tensor, device="cuda"):
    if _is_vavae(vae):
        ...
        latent = vae.encode(image_tensor).sample()
        # No scaling factor for VAVAE
        return latent
...
def decode_latent(vae, latent, device="cuda", scaling_factor=None):
    scale = float(_vae_scaling_factor(vae) if scaling_factor is None else scaling_factor)
    if _is_vavae(vae):
        # VAVAE requires float32 input, decode() returns tensor directly
        latent_f32 = (latent / max(scale, 1e-8)).to(device, dtype=torch.float32)
        image = vae.decode(latent_f32)
    else:
