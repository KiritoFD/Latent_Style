...
\begin{abstract}
We propose Latent Bridge Matching, an efficient framework for domain-level artistic transfer that learns a time-conditioned velocity field in compact VAE latent space. Rather than optimizing per-image or sampling a large diffusion model, our method constructs OT-coupled content--style latent pairs using a sliced-Wasserstein transport cost, trains a latent flow-matching objective at randomly sampled bridge times, and regularizes the integrated endpoint with terminal distribution matching. This separates two forces: the OT/SWD terminal objective drives the output toward the target artistic domain, while the latent flow and kinetic regularization constrain the path to preserve content structure. Under a strict 750-output protocol over five domains, our 3.9M-parameter model reaches competitive CLIP-style performance, improves LPIPS and EC over SaMST, S2WAT, AdaIN, CAST, and StyleID, and trains substantially faster than heavier feed-forward baselines. The results suggest that OT-coupled latent flow matching provides a clean, reproducible efficiency-quality trade-off for domain-level artistic transfer.
\end{abstract}
...
\begin{itemize}
    \item We formulate domain-level artistic transfer as \textbf{OT-coupled latent flow matching}, where unpaired content and target-style latents are matched with a sliced-Wasserstein transport cost and used to supervise a time-conditioned velocity field.
    \item We introduce a lightweight domain-conditioned latent backbone with learned style embeddings, spatial priors, and semantic routing, enabling style transfer without per-image optimization, reference-image encoding, or diffusion sampling.
    \item We combine flow-matching supervision with terminal SWD endpoint regularization, explicitly separating path/content preservation from target-domain distribution alignment.
    \item We evaluate under a strict 750-output protocol and show that our method lies on a favorable efficiency-quality Pareto frontier: 3.9M parameters, competitive CLIP-style, lower LPIPS, best EC among compared methods, and substantially lower training cost.
\end{itemize}
...
Table~\ref{tab:main} reveals a clear style-content-efficiency trade-off. StyleID obtains the highest raw CLIP-style score, but its LPIPS of 0.750 indicates severe content drift and its diffusion-based inference is over 15$\times$ slower. SaMST is the closest efficient competitor and slightly exceeds our CLIP-style by 0.003; however, Ours achieves lower LPIPS (0.451 vs 0.466), higher EC (0.393 vs 0.384), fewer parameters (3.9M vs 6.0M), and substantially lower training cost (310 s vs 6769 s). S2WAT reaches a similar CLIP-style but has much worse LPIPS (0.526) and a 16.7$\times$ larger model. AdaIN remains the fastest method, but its LPIPS and EC are far behind (0.630, 0.264). CAST and diffusion-based StyleID are constrained by content drift or high inference cost. These results support our main claim: Ours does not maximize a single metric alone, but provides a favorable Pareto point for efficient domain-level artistic transfer.
...
The experiments support a specific and conservative conclusion: the proposed method lies on a favorable efficiency-quality Pareto frontier for domain-level artistic transfer. Ours does not achieve the single highest raw CLIP-style---StyleID attains 0.760 at the cost of content collapse, and SaMST is 0.003 ahead with heavier parameters and much longer training. However, Ours combines competitive style quality (CLIP-style 0.716) with the lowest LPIPS among high-style methods, the best EC score (0.393), the smallest model (3.9M), and the shortest measured training time (310 s). This joint efficiency-quality profile is the paper's main finding.
