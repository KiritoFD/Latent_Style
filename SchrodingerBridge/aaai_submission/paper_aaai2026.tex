...
\begin{itemize}
    \item \textbf{We formulate domain-level artistic transfer as executable latent transport}, separating style specification from latent execution through training-side endpoint pairing, time-conditioned field learning, explicit motion control, and terminal distribution matching.
...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.96\linewidth]{figures/fig_distinct5_same_cost_performance.png}
    \caption{Matched-budget Distinct5-512 same-cost performance plot. Higher and further right is better: transfer CLIP-S is plotted against $1-\mathrm{LPIPS}$, and the dashed line marks the transfer-only IDT floor. Under roughly two-minute training budgets, LBM stays in the favorable upper-right positive-IDT region, same-cost SaMST sits only slightly above IDT at much larger displacement, and same-cost SaMAM remains below IDT.}
    \label{fig:samecostperf}
\end{figure}

\paragraph{Adaptation cost and accessibility.}
Table~\ref{tab:cost_distinct5}, Figure~\ref{fig:samecostperf}, and Figure~\ref{fig:timecontext} add a matched same-cost rerun on top of the retained Distinct5 frontier. Figure~\ref{fig:samecostperf} is the quality view of that rerun itself: after roughly two minutes of training, the LBM step-350 row reaches transfer CLIP-S 0.6629 at LPIPS 0.3377 and sits +0.0230 above the Distinct5 IDT floor. That row is intentionally a matched-budget operating point rather than the strongest retained LBM operating point: the reviewed LBM frontier already appears earlier, around 1.2 minutes, at LBM-F and LBM-K. Under the same matched-budget rerun, SaMST reaches only +0.0166 while paying a much larger LPIPS cost of 0.7487 and a slower 461\,ms/image executor, and SaMAM remains below IDT at $-0.1383$. Measured local GPU traces keep those matched-budget runs within the same 8\,GB RTX~4070 laptop envelope, peaking at about 5.0\,GB for LBM, 6.1\,GB for SaMST, and 7.5\,GB for SaMAM. Figure~\ref{fig:timecontext} then places the same rerun back into the longer frontier history instead of presenting it as a closed normalized time-to-parity theorem. The safe reading is therefore an evidence-closed operating-point claim rather than a universal time-to-parity claim: within this domain-style setting, compact latent transport can shift customized stylization from a resource-intensive adaptation problem toward a minute-scale small-model learning problem. This matters beyond leaderboard optics because it lowers the barrier for small labs, independent researchers, and creators who want style-specific models without adapting a large generative prior.
...
\begin{figure}[t]
    ...
    \caption{Evidence-closed Distinct5 timing records plotted as transfer-only $\Delta_{\mathrm{idt}}$ against cumulative training wall time under the reproduced full $5\times5$ / 750-output protocol. The horizontal axis is log-scaled and inverted so faster adaptation appears to the right. LBM is shown in two roles: the retained reviewed frontier points around 1.2 minutes and the later step-350 matched-budget rerun. SaMAM is shown both as the exact same-cost row and as bounded pre-2250 timing records with the active manuscript boundary fixed at the trusted 2250 checkpoint; SaMST is shown both as the exact same-cost row and as the longer retained operating point. The plot therefore preserves the calibration between the retained frontier and the matched-budget rerun rather than serving as a closed normalized time-to-parity claim.}
