...
\begin{abstract}
Domain-level artistic transfer must balance three objectives that often conflict: target-style strength, content preservation, and computational cost. We propose Latent Bridge Matching (LBM), a compact latent-space framework that treats stylization as controlled transport in a VAE latent manifold rather than as direct latent reconstruction. The model couples mini-batch optimal-transport endpoint assignment, time-conditioned transport prediction, semantic routing, and the SA-SWD terminal design used in the current mainline. In this view, the style tokenizer specifies what target style should be executed, while the LANCET backbone realizes that control as a latent vector field. Under a strict 750-output protocol over five domains, a 3.9M-parameter LBM model reaches near-SaMST CLIP-style (0.716 vs.\ 0.719), improves LPIPS and EC, and remains practical to evaluate, with strict-750 inference in 85.4\,s (114\,ms/image) on a single RTX 4070 Laptop GPU. On Distinct5-512, an ordinary WikiArt split formed only by selecting the five most separated style domains under a CLIP-style screening criterion, the best LBM checkpoints stay above the unchanged-image reference and define the strongest measured CLIP-S / LPIPS trade-off among the currently reproduced positive-$\Delta_{\mathrm{idt}}$ full 5$\times$5 / 750-output points: LBM-F reaches CLIP-S 0.6969 and LPIPS 0.3186, while reproduced SaMAM points stay below the no-op floor despite substantially improving target-wise ArtFID, and SaMST-512 attains higher raw style only with severe LPIPS and ArtFID degradation. Taken together, the results expose a serious evaluation pathology rather than a universal dismissal of prior work: on a normal but well-separated art-to-art split, common absolute metrics can reward the wrong success condition, namely structure-preserving, art-domain-plausible outputs that still fail to move toward the requested target style beyond an unchanged-image control.
\end{abstract}

\section{Introduction}
Artistic style transfer has long served as a testbed for separating image content from visual appearance. Classical neural style transfer showed that convolutional features and Gram statistics can synthesize compelling artistic textures \cite{gatys2016image}, while feed-forward arbitrary style transfer made real-time deployment practical through normalization-based style modulation \cite{huang2017adain}. Subsequent attention and transformer systems improved local correspondence and long-range context \cite{park2019sanet,deng2022stytr2,zheng2024puffnet}. Recent work has since split into three increasingly distinct inference-time regimes. First, reference-guided arbitrary stylization still receives a style exemplar at inference and refines how that exemplar is fused, including perceptual-quality refinement in AesPA/AesFA \cite{hong2023aespa,huang2024aesfa}, more structured style injection in HSI \cite{zhang2025hsi}, and semantic-region consistency in SCSA \cite{shang2025scsa}. Second, compact multi-style methods amortize a fixed style family into lightweight branches or codes that can be selected by style identity alone. Third, training-free VQ and diffusion-based methods strengthen style expression by reusing larger priors without retraining \cite{gim2024cast,chung2024styleid,deng2024zstar,jiang2025sms,xu2025stylessp}. Across these regimes, practical artistic transfer still exposes a persistent three-way tension: strong target-style expression, faithful content preservation, and low computational cost.

This tension is especially visible in domain-level multi-style transfer. Our setting is closest to the compact multi-style regime: inference receives only a target style/domain identifier, not a target style image. This sharply differs from reference-guided systems, which center exemplar-conditioned correspondence or injection strength \cite{zheng2024puffnet,zhang2025hsi,shang2025scsa,chung2024styleid,deng2024zstar}, and also from state-space variants that still target reference-guided efficiency through alternative global-mixing operators \cite{liu2025samam,botti2025mambast}. In the compact style-id regime, the bottleneck is not exemplar matching but executing a stable style control signal without washing out content or collapsing into texture artifacts. Under our strict 750-output protocol, StyleID attains the highest raw CLIP-style score but exhibits severe content collapse, while SaMST achieves strong raw style and content scores yet produces visible muddy and grain-like texture artifacts. These observations suggest that standard style/content metrics alone are insufficient. A competitive system must also be evaluated for perceptual quality and artifact-sensitive texture realism. In particular, low-frequency structure metrics can overestimate perceptual quality when local texture becomes muddy or grain-like.
...
\begin{itemize}
    ...
    \item \textbf{A documented current-mainline design choice for semantic-aligned terminal matching (SA-SWD)}, which converts cross-attention routing evidence into terminal distribution-matching axes while retaining the efficiency of SWD; its semantic-axis necessity is not treated as a positive closed claim here and is instead bounded by a landed matched semantic-vs-random control that closes negatively on the current Distinct5 packet.
    \item \textbf{An executable style-tokenizer formulation with bounded localization diagnostics}, separating the style-side representation from the latent renderer. The tokenizer receives style identity and learned style statistics, never per-sample target evidence in the main style-id protocol; successor-family probes and a matched localization packet are then used only to test whether learned controls remain distinct after the LANCET vector field executes them.
    ...
    \item \textbf{A separated Distinct5-512 stress benchmark}, used as a metric stress test rather than a universal art benchmark, on which the best LBM variants define the strongest measured CLIP-S / LPIPS trade-off among the currently reproduced positive-$\Delta_{\mathrm{idt}}$ points.
...
\begin{figure*}[t]
    \centering
    \includegraphics[width=0.94\textwidth]{figures/fig_distinct5_pareto.pdf}
    \caption{Distinct5-512 style-content frontier. Panel (a) shows the full evaluated trajectory and operating-point cloud under the reproduced 5$\times$5 / 750-output protocol. Panel (b) zooms into the frontier region used for direct LBM-vs-baseline operating-point comparison; it is not a separate metric or filtered evaluation set. The dashed horizontal line marks the \emph{idt} no-op CLIP-style level. LBM points are the best tokenizer-queue variants, the SaMAM curve shows cumulative training progress, and the SaMST-512 point marks a high-style, high-damage regime.}
    \label{fig:distinct5}
\end{figure*}

Distinct5-512 is a metric-stress benchmark rather than a universal art benchmark. It is not a contrived out-of-distribution trap or a failure-mined adversarial set: all images come from standard WikiArt classes, and the five domains are selected only because they are among the most separated under a CLIP-style screening pass. Figure~\ref{fig:distinct5} is the main result view, and Table~\ref{tab:distinct5} gives the exact numbers. The comparison is stricter than a single target direction: every method must handle identity directions, nearby art-to-art transfers, and distant cross-style transfers. That is not an exotic demand; it is the minimum sanity condition for an art-to-art metric. If a method is asked to move between deliberately separated WikiArt styles, unchanged output should not be allowed to masquerade as success. On this split, however, the \emph{idt} baseline already reaches CLIP-S 0.6801 while doing no transfer at all, so raw CLIP-style alone overstates performance. Under the current Distinct5-512 full 5$\times$5 / 750-output protocol, LBM occupies the strongest measured CLIP-S / LPIPS trade-off among the currently reproduced positive-$\Delta_{\mathrm{idt}}$ operating points. The content-preserving LBM-F checkpoint reaches CLIP-S 0.6969 and LPIPS 0.3186. The timing entries in Table~\ref{tab:distinct5} are recorded operating-point wall clocks and are included as context only, not as normalized time-to-parity evidence. SaMST-512 shows a different failure mode: raw style rises to 0.7247, but LPIPS degrades to 0.6255 and ArtFID worsens to 395.7. The style-seeking LBM-K checkpoint raises CLIP-S to 0.7010 while remaining far below that high-damage region. In no-op-adjusted terms, LBM remains positive under both scopes: LBM-F gives $\Delta_{\mathrm{idt}}=+0.0168$ full and $+0.0244$ transfer-only, while LBM-K gives $+0.0209$ full and $+0.0312$ transfer-only. By contrast, SaMAM stays below the idt floor throughout the evaluated run even though its reproduced target-wise ArtFID improves from 216.5 to 146.1 on the full scope and from 323.7 to 148.2 on the transfer-only scope. This is the sharp failure case: the reproduced model is not merely producing arbitrary failures; it pays distortion budget, reaches a lower-ArtFID regime, and still fails to achieve positive target-style gain beyond idt. On Distinct5, lower ArtFID can therefore reflect source-structure preservation and generic target-domain plausibility while still failing the requested target-style transfer. The critique is not that a CVPR baseline is globally invalid; it is that the common absolute-score protocol can certify the wrong success condition for art-to-art transfer on an ordinary separated WikiArt split. SaMST reaches positive style gain only in a high-LPIPS / high-ArtFID regime. This section should therefore be read as a frontier comparison on the current reproduced points, not as proof that semantic projection-axis selection is already isolated as the decisive cause.
...
All reported evaluations use fixed seeds, stored resolved configurations, source snapshots, checkpoints, and per-epoch logs. Historical timings were measured on an RTX 4070 Laptop GPU unless otherwise noted. For Distinct5-512, we maintain a separate same-scope timing artifact built from reviewed operating-point records and partial curves, with cumulative training wall time plotted while evaluation is excluded. We use that artifact only as bounded timing context and do not elevate it to a normalized parity claim in the main paper. For the historical table, training and inference cost are reported explicitly in Table~\ref{tab:cost} rather than being folded into a single efficiency claim.
...
The tokenizer experiments narrow the design question. Within the tested Distinct5 tokenizer variants, a larger style code or more class-local prototypes is not sufficient. The sharper open issue in this family is whether target-style information survives execution through a content-conditioned vector field strongly enough to yield real no-op-adjusted style gain. The current evidence therefore favors target-style carriers, prototype-aware target queues, and content-aware routing over black-box token capacity in this setting. A landed matched Distinct5 successor probe sharpens that boundary further: tokenizer code separation maps only moderately into executed separation, while no-op-adjusted style gain tracks executed movement more closely than raw code geometry. A separate matched `L e1` localization packet then suggests that executor-side refresh is the stronger one-sided recovery direction on that reviewed surface, again without restoring H-family continuity or ruling out tokenizer-side weakness. It also shows a trade-off: content-adaptive atom routing can increase style strength but may increase endpoint movement and LPIPS. A plausible next-stage tokenizer direction is therefore to factor target representation from execution budget rather than simply adding parameters.
