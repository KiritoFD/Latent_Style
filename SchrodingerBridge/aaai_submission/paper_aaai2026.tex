...
\begin{itemize}
    ...
    \item \textbf{We expose the Identical-Image Transfer (IDT) floor as a required control}, showing on Distinct5-512 that a reproduced baseline can visibly edit images yet score below IDT in transfer-only CLIP-S, and advocating $\Delta_{\mathrm{idt}}$ as a standard diagnostic for domain-level art-to-art evaluation.
...
\end{equation}
where the SWD compares generated endpoint patches with coupled target patches. In the current mainline configuration, we use semantic cues only as a projection-selection heuristic rather than as a separate claim of semantic optimality. Let $K_s$ denote the keys produced by semantic cross-attention for target style $s$. We rank spatial target positions by the average magnitude of $K_s$ and use the corresponding target latent patch vectors as normalized projection directions. This defines the SA-SWD instantiation used in the current mainline: terminal distribution matching is still computed by sorted one-dimensional projections, but the projection axes are adaptively chosen from the same routing signal that injects style into the backbone.
...
The transport design admits a compact principled analysis (Sec.~\ref{sec:formal}):
...
\begin{figure*}[t]
    ...
    \caption{Historical strict-750 qualitative source-target grids for LBM epoch 7 and SaMST strict at 256 resolution. Rows indicate source domains and columns indicate target domains. SaMST preserves coarse structure but often produces muddy, grain-like surface texture, whereas LBM maintains cleaner and more coherent brush structure on the same directions.}
    \label{fig:qual_grid}
\end{figure*}
...
\begin{figure}[t]
    ...
    \caption{Centered texture crops from matched 256-resolution LBM and SaMST outputs, isolating the local surface-texture failure mode: SaMST introduces muddy high-frequency grain, whereas LBM preserves cleaner contiguous brush traces.}
    \label{fig:zoom}
\end{figure}
