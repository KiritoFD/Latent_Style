...
Table~\ref{tab:cost_distinct5} and Figure~\ref{fig:timecontext} add a matched same-cost rerun on top of the retained Distinct5 frontier. The quality view of that rerun already appears on page~1, so the main text keeps only the complementary calibration view: after roughly two minutes of training, the LBM step-350 row reaches transfer CLIP-S 0.6629 at LPIPS 0.3377 and sits +0.0230 above the Distinct5 IDT floor. That row is intentionally a matched-budget operating point rather than the strongest retained LBM operating point: the reviewed LBM frontier already appears earlier, around 1.2 minutes, at LBM-F and LBM-K. Under the same matched-budget rerun, SaMST reaches only +0.0166 while paying a much larger LPIPS cost of 0.7487 and a slower 461\,ms/image executor, and SaMAM remains below IDT at $-0.1383$. Measured local GPU traces keep those matched-budget runs within the same 8\,GB RTX~4070 laptop envelope, peaking at about 5.0\,GB for LBM, 6.1\,GB for SaMST, and 7.5\,GB for SaMAM. Figure~\ref{fig:timecontext} therefore serves a different purpose from the teaser: it places the same rerun back into the longer frontier history instead of repeating the same page-1 quality scatter. The safe reading is therefore an evidence-closed operating-point claim rather than a universal time-to-parity claim: within this domain-style setting, compact latent transport can shift customized stylization from a resource-intensive adaptation problem toward a minute-scale small-model learning problem. This matters beyond leaderboard optics because it lowers the barrier for small labs, independent researchers, and creators who want style-specific models without adapting a large generative prior.
