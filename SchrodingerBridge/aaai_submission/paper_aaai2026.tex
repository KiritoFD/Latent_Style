...
\begin{figure*}[t!]
    ...
    \caption{Overview of Latent Bridge Matching. The top band shows style-side control, where the style tokenizer maps the target style identifier into a compact style code. The middle band is the active inference path: content is encoded into a latent, passed through the LBM time-conditioned residual field with semantic routing, integrated by $K$-step Euler updates, and decoded into the stylized output. The bottom band summarizes training-only constraints: in the current Distinct5 mainline, target-domain latents enter through pairing-cache endpoint selection with OMF-style endpoint-delta supervision, while the broader bridge lineage also includes OT/Sinkhorn parent variants. The SA-SWD inset makes explicit how semantic bins define terminal within-bin SWD matching. Target-domain latents are therefore training-side supervision only, not inference-time conditioning inputs.}
    \label{fig:framework}
\end{figure*}
...
The core of the method is a time-conditioned velocity field $v_\theta(z_t,t,s)$ that executes style-conditioned latent motion between selected training endpoints. The broader bridge lineage studies stochastic interpolation and flow-style supervision. At a randomly sampled time $t\sim\mathcal{U}(0,1)$, the interpolated state is
\begin{equation}
...
\end{equation}
Here $\rho$ is the pointwise velocity penalty. The implementation supports MSE, Huber, and $L_1$ variants in direct velocity-regression branches. Equation~(5) defines the historical parent transport objective; in the reproduced Distinct5 setting, training resolves to OMF-style endpoint-delta supervision together with terminal matching and kinetic regularization. In other words, Eq.~(5) is design-family background rather than the literal active headline loss for the Distinct5 rows.
...
the current Distinct5 mainline, while Theorem~3 concerns a
historical OT-style endpoint-assignment variant from the broader
LBM family. Full proofs appear in the supplement.
...
This theorem concerns the historical OT-style parent variant rather than the literal active Distinct5 headline loop.
...
Cost matters here as deployment evidence, not as a normalized time-to-parity contest. The evidence-closed historical rows are 5.2 minutes of training to the reported LBM checkpoint versus an estimated 1.9 hours to the reproduced SaMST operating point, with strict-750 inference at 114\,ms/image for LBM and 53\,ms/image for SaMST. The practical reading is straightforward: LBM is not the fastest feed-forward stylizer in absolute milliseconds, but it remains in a compact single-image interactive regime at 114\,ms/image while keeping a 3.91M-parameter footprint. We therefore report cost as explicit operating-point context instead of collapsing unlike training protocols into a single efficiency slogan.
