...
Table~\ref{tab:cost_distinct5} reports the closed same-cost rows, and Table~\ref{tab:distinct5pairs} shows representative transfer directions.

A paired bootstrap over the 600 transfer rows gives 95\% intervals of $[0.0210, 0.0280]$ for LBM-F and $[0.0273, 0.0352]$ for LBM-K, confirming stable positive transfer-only $\Delta_{\mathrm{idt}}$ on this split. SaMST also stays above IDT in the bootstrap, but only in the same high-LPIPS regime.

\begin{table}[t]
\centering
\caption{Representative Distinct5 transfer-only pairs. Cells report CLIP-S / LPIPS. ER = Early Renaissance, Imp = Impressionism, Min = Minimalism, Roc = Rococo, Ukiyo = Ukiyo-e.}
