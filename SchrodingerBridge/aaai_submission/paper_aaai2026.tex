...
\begin{figure*}[t]
    ...
    \caption{Distinct5-512 style-content frontier. Panel (a) shows the reproduced operating-point cloud under the 5$\times$5 / 750-output protocol. Panel (b) zooms into the frontier region used for direct LBM-vs-baseline operating-point comparison; it is not a separate metric or filtered evaluation set. The dashed horizontal line marks the \emph{idt} no-op CLIP-style level. LBM points are the best tokenizer-queue variants, the SaMAM point is the trusted 2250-step checkpoint, and the SaMST-512 point marks a high-style, high-damage regime.}
    \label{fig:distinct5}
\end{figure*}
