...
\begin{abstract}
Domain-level artistic transfer has a blunt evaluation failure: when the source is already art, doing nothing can score as target style. On the CLIP-separated WikiArt Distinct5 stress split, the reproduced SaMAM run remains below the identical-image transfer-only CLIP-S floor through its last trusted 2250-step checkpoint. We introduce \emph{Latent Bridge Matching} (LBM), a 3.9M-parameter latent-transport model that selects target-domain endpoints during training, executes a style-conditioned vector field, and applies terminal distribution matching in VAE latent space. On Distinct5-512, the reviewed LBM-F/K operating points exceed IDT by +0.0244/+0.0312 transfer-only CLIP-S at the first reproduced RTX 3060 checkpoint (1.2 minutes), whereas SaMST exceeds the same floor only in a much higher-displacement, higher-targetwise-ArtFID region. On the historical strict-750 reference surface, LBM reaches 0.7161 CLIP-S / 0.4514 LPIPS at 114\,ms/image, motivating a simple rule for separated art-to-art stress splits of this type: measure the unchanged-image floor before treating raw target-style affinity as stylization.
\end{abstract}
...
\captionof{figure}{IDT-calibrated Distinct5-512 transfer-only operating points. Among the reproduced comparison points shown here, SaMAM stays below IDT, SaMST exceeds IDT only in a high-displacement, high-targetwise-ArtFID region, and LBM-F/K exceed IDT in a lower-damage region.}
...
We propose \emph{Latent Bridge Matching} (LBM), a compact latent-transport model built around that evaluation requirement. LBM combines (i) entropic OT couplings that construct unpaired latent endpoint pairs; (ii) a style-conditioned latent vector field learned by conditional flow matching; and (iii) SA-SWD, an adaptive-projection terminal regularizer that pulls the Euler-integrated endpoint toward target-style patch statistics. A learnable style spatial prior and semantic cross-attention provide domain-level conditioning without per-image optimization or diffusion sampling. The paper also establishes a reporting rule for separated art-to-art stress splits of this type: evaluate style gain against the identical-image floor, not raw target-style affinity alone.

Our contributions are:
\begin{itemize}
    \item \textbf{We introduce LBM, a compact latent-transport design for domain-level artistic transfer}, combining entropic OT couplings, conditional vector-field execution, and terminal distribution matching as an alternative to direct latent endpoint reconstruction.
    \item \textbf{We expose the Identical-Image Transfer (IDT) floor as a necessary diagnostic on separated art-to-art splits}, showing on Distinct5-512 that a reproduced baseline can visibly edit images yet score below IDT in transfer-only CLIP-S, and advocating $\Delta_{\mathrm{idt}}$ as the companion metric that measures genuine style movement beyond that floor.
...
The core of the method is a time-conditioned velocity field $v_\theta(z_t,t,s)$ that transports latents between OT-coupled endpoints along a stochastic interpolation path. At a randomly sampled time $t\sim\mathcal{U}(0,1)$, the interpolated state is
\begin{equation}
...
\end{equation}
Here $\rho$ is the pointwise velocity penalty. The implementation supports MSE, Huber, and $L_1$ variants in the direct velocity-regression path. Equation~(5) defines the parent transport objective; in the reproduced Distinct5 setting, training instantiates that objective through OMF-style endpoint-delta supervision together with terminal matching and kinetic regularization. The current evidence favors this OT + $W_1$-style mainline over isolated endpoint-only pointwise supervision.
...
For execution, the backbone may also use style spatial priors and content-conditioned routing. These modules modulate where a target-style control is applied, not what target style is requested. This distinction matches the empirical diagnosis: the key question is not raw tokenizer capacity in isolation, but whether the style carrier remains executable after it passes through a content-conditioned renderer. Prototype-aware target queues and content-guided routing improve that executed style-content frontier, which is why we phrase tokenizer evidence at the level of \emph{executable representation}: code geometry matters only insofar as generated latent residuals remain distinguishable after LANCET consumes the code.

\paragraph{Complementary roles.}
The components address distinct subproblems. OT coupling provides target-style endpoints for unpaired supervision, but not transport dynamics. Transport supervision learns how to move toward those endpoints, but by itself does not guarantee that the integrated endpoint matches target-domain texture statistics. SA-SWD regularizes this endpoint mismatch, while semantic cross-attention provides local routing for where style is written and supplies a principled heuristic for projection selection. The destructive ablations directly verify the roles of terminal distribution matching and kinetic regularization. On the current Distinct5 packet, the dominant gain comes from distributional terminal matching itself, which avoids pointwise latent collapse, while semantic routing refines how those projection axes are chosen.
...
Under bounded temporal drift and Lipschitz continuity, endpoint kinetic regularization acts as a tractable proxy for inference-time path action. Theorem~1 formalizes why a lightweight endpoint penalty is sufficient to straighten trajectories in the operating regime studied here, without requiring expensive multi-step supervision.
...
Because VAE latents exhibit non-trivial local curvature, few-step solvers can otherwise accumulate visible drift. Here the measured $O(1/K)$ decay shows that the default few-step Euler regime remains strictly controlled in practice. At $K=12$ (the default), the error is $<0.6$---negligible relative to the style-transfer effect.
...
\toprule
Method & CLIP-S$\uparrow$ & LPIPS$\downarrow$ & EC$\uparrow$ & tw-ArtFID$\downarrow$ & $\Delta_{\mathrm{idt}}\uparrow$ & $\Delta_{\mathrm{idt,tr}}\uparrow$ & \begin{tabular}[c]{@{}c@{}}Wall time\\to point\end{tabular} \\
\midrule
...
Table~\ref{tab:distinct5pairs} makes the frontier interpretation concrete on representative transfer-only pairs from the reviewed packet: SaMST can produce locally strong target-style jumps, but those gains are not stable across directions and often require much larger displacement than the balanced LBM-F point, while SaMAM still often edits without moving toward the requested target style.
...
Cost matters here as deployment evidence, not as a normalized time-to-parity contest. The reproduced operating-point records are 310\,s for the reported LBM checkpoint, 6769\,s for the reproduced SaMST operating point, and 10600\,s for S2WAT. Historical strict-750 inference is 114\,ms/image for LBM, 53\,ms/image for SaMST, and 804\,ms/image for StyleID. The practical reading is straightforward: LBM is not the fastest feed-forward stylizer in absolute milliseconds, but it remains in a compact single-image interactive regime at 114\,ms/image while avoiding StyleID's diffusion-scale inference cost and keeping a 3.91M-parameter footprint. We therefore report cost as explicit operating-point context instead of collapsing unlike training protocols into a single efficiency slogan.

\subsection{Artifact-sensitive diagnosis}
The artifact claim is clearest on the historical 256-resolution packet, where SaMST is the closest strict-750 efficient baseline to LBM on the core metrics and also the closest recovered baseline with locally readable outputs for crop-based diagnosis. Figure~\ref{fig:artifact_panel} uses fixed matched crop boxes rather than free-floating zooms. The top grids show the full source-target matrix, while the lower row magnifies the exact boxed regions. On these matched cells, SaMST often preserves coarse layout but fills high-frequency regions with muddy grain, line-boundary contamination, and broken local brush continuity; LBM is less aggressive in raw style, but cleaner on the same spatial evidence.

\begin{figure*}[t]
    \centering
    \includegraphics[width=0.92\textwidth]{figures/fig_artifact_panel_ours_vs_samst.png}
    \caption{Matched 256-resolution artifact diagnosis on the historical strict-750 packet. Top: full source-target grids for LBM epoch 7 and SaMST with three fixed crop boxes. Bottom: magnified crops from the exact boxed regions. SaMST is shown as the closest efficient strict-750 256-resolution baseline with valid local outputs; boxes A--C highlight luminous breakup, line-boundary contamination, and body-contour corruption, respectively.}
    \label{fig:artifact_panel}
\end{figure*}

\begin{table}[t]
\centering
\caption{Artifact-sensitive comparison on the historical strict-750 packet among high-quality methods.}
\label{tab:artifact}
\small
\begin{tabular}{lrrr}
\toprule
...
\section{Conclusion}
We presented Latent Bridge Matching, an OT-coupled latent transport model for compact domain-level artistic transfer. The method separates endpoint construction, path-wise field learning, adaptive-projection terminal distribution matching, and style-tokenized execution. Under the reproduced historical strict-750 protocol, LBM delivers the cleanest reproduced balance of style strength, content fidelity, artifact control, and practical footprint within the mixed-family comparison packet considered here. The Distinct5-512 stress benchmark sharpens the paper's main evaluation point: once the identical-image prior is made explicit, LBM stays above the no-op floor and defines the strongest measured positive-$\Delta_{\mathrm{idt}}$ CLIP-S / LPIPS frontier among the reproduced compact operating points, while lower ArtFID alone does not guarantee useful target-style movement. For separated art-to-art stress splits of this type, reporting the IDT floor together with $\Delta_{\mathrm{idt}}$ is therefore a necessary diagnostic rather than an optional extra statistic. The tokenizer study then isolates the remaining mechanism question. Within the tested Distinct5 tokenizer variants, matched localization controls show that executor-side refresh recovers more than style-side refresh alone on the current local surface, while the same-family H path-stability packet supports the bounded claim that kinetic regularization acts as a practical stabilizer in the current OMF / field regime. The next representation target is therefore to preserve target-style separation through execution itself, via explicit target carriers and tighter content-risk control.
