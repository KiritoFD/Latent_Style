...
\begin{abstract}
Domain-level artistic transfer must balance three objectives that often conflict: target-style strength, content preservation, and computational cost. We propose Latent Bridge Matching (LBM), a compact latent-space framework that treats stylization as controlled transport in a VAE latent manifold rather than as direct latent reconstruction. The model couples mini-batch optimal-transport endpoint assignment, time-conditioned transport prediction, semantic routing, and a semantic-aligned sliced Wasserstein (SA-SWD) terminal design. In this view, the style tokenizer specifies what target style should be executed, while the LANCET backbone realizes that control as a latent vector field. Under a strict 750-output protocol over five domains, a 3.9M-parameter LBM model reaches near-SaMST CLIP-style (0.716 vs.\ 0.719), improves LPIPS and EC, and remains practical to evaluate, with strict-750 inference in 85.4\,s (114\,ms/image) on a single RTX 4070 Laptop GPU. On the Distinct5-512 metric-stress split built from the five most separated WikiArt style domains under a CLIP-style screening criterion, the best LBM checkpoints stay above the unchanged-image reference and define the strongest measured content-preserving frontier among the currently reproduced points: LBM-F reaches CLIP-S 0.6969 and LPIPS 0.3186, while reproduced SaMAM points stay below the no-op floor and SaMST-512 attains higher raw style only with severe LPIPS and ArtFID degradation. Taken together, the results show that raw CLIP-style is sensitive to the unchanged-image prior on separated art-to-art transfer, and that the main remaining bottleneck is not tokenizer size, but faithful style execution through the latent renderer.
\end{abstract}
...
\begin{itemize}
    ...
    \item \textbf{A semantic-aligned terminal matching design (SA-SWD)}, which converts cross-attention routing evidence into terminal distribution-matching axes while retaining the efficiency of SWD; the current paper evaluates this mechanism as a proposed mainline design rather than treating semantic-axis necessity as already closed.
