...
experiments reported in the main paper. It consolidates the local paper directory,
remote experiment records, frozen run configurations, training logs, evaluation summaries,
...
\texttt{<EXP\_ROOT>}, and \texttt{<CACHE\_ROOT>}; no workstation-local or user-local
paths are required by the protocol.
\end{abstract}
...
\begin{itemize}[leftmargin=1.5em,itemsep=0.2em]
  ...
  \item \texttt{exp/dino\_s\_break/brk\_a\_ll03\_10ep/config.json}: frozen merged remote run configuration.
  \item \texttt{exp/dino\_s\_break/brk\_a\_ll03\_10ep/logs/training\_20260712\_214129.csv}: remote training log.
  ...
  \item \texttt{docs/713/REPO\_REMOTE\_AUDIT\_2026-07-13.md}: internal local/remote audit used to sanitize public paths.
\end{itemize}

All paths above are relative to either \texttt{<REPO\_ROOT>/SchrodingerBridge} or
\texttt{<EXP\_ROOT>}. The experiments were audited on the remote experiment drive and
then rewritten here without absolute drive letters, usernames, hostnames, or cache paths.

\subsection{Directory Audit and Cleanup Policy}

Before writing this supplement, the paper directory and remote experiment drive were
audited rather than treated as a single opaque result folder. The audited remote
...
The local source checkout was also audited and found to be an active research worktree
...
\caption{Audited remote experiment-tree inventory. The paper-facing records are a small
...
\toprule
Remote group & Count & Interpretation \\
\midrule
...
\end{tabular}
\end{center}
No absolute drive letters, SSH addresses, workstation names, or user names are needed to
run the protocol.
...
drive letters and remote host details are not part of the experiment definition.}
...
The primary hardware for the paper training and speed measurements is a remote consumer
GPU workstation:
...
GPU memory & 12 GB \\
Driver family & 581.xx / CUDA-capable Windows driver \\
Operating system & Windows remote workstation \\
Training precision & bfloat16 automatic mixed precision \\
TF32 & enabled \\
Primary train device count & 1 GPU \\
\bottomrule
\end{tabular}
\end{center}
...
The frozen remote merged configuration for the paper run resolves the short public config
...
roughly 17.1 s/epoch on the remote RTX 3060.
...
\path{src/utils/run_evaluation.py} for board evaluation. Each remote training run copies
...
\midrule
Baseline style memory & \texttt{style\_id} \(\rightarrow\) style memory \(\rightarrow\) cross-attention & Auxiliary. Gates stay near 0.056--0.061 and gradient mass on style memory is weak. \\
...
\caption{Generation-only timing decomposition on the remote RTX 3060. VAE decode is a
...
The audited generation-only timing on the remote RTX 3060 is:
...
\begin{itemize}[leftmargin=1.5em,itemsep=0.2em]
  ...
  parameter count and training minutes are reported when locally measured.
...
The baseline ledger contains both measured local baselines and externally sourced or
...
Latent-WCT & local analytic transform & Zero trained parameters; evaluates whether wavelet statistics alone are sufficient. \\
CUT / SaMST / SaMam & local learned baselines & Locally trained or re-evaluated where checkpoints/outputs were available; train/infer times are measured on comparable hardware when reported. \\
SD-Turbo / StyleAligned / Z-STAR / StyleShot & frozen-backbone editing baselines & Large pretrained backbones; no task-specific training counted, but inference wall-clock is reported when outputs were generated locally. \\
...
WEAVE & remote RTX 3060 run & Main checkpoint and timing records come from the audited remote experiment tree. \\
\bottomrule
\end{tabularx}
\normalsize
\end{center}
...
endpoint statistics and wavelet-domain transport are active, while a large learned
cross-attention memory is not the main source of style transfer. This is why the paper
frames \weave{} as a compact wavelet-flow model rather than as a transformer-style
...
around 0.056--0.061 and weak gradient mass on the style-memory branch. The effective
style movement comes mainly from endpoint AdaIN/statistics and flow transport rather than
from a large cross-attention style memory.
...
\begin{itemize}[leftmargin=1.5em,itemsep=0.2em]
  ...
  local/global HF injection, and target-HF route variants;
...
\[
  ...
  \rightarrow\mathrm{multi\mbox{-}token\ subband\ code}\\
...
\midrule
Multi-token per-subband codes & Keep & Increases style capacity without exposing target coordinates. \\
...
equivalent local or remote storage.
...
\begin{enumerate}[leftmargin=1.6em,itemsep=0.2em]
  ...
  \item Confirm no public supplement source contains absolute paths, SSH endpoints, or
  usernames.
\end{enumerate}
...
\path{exp/dino_s_break/brk_a_ll03_10ep/config.json} & frozen merged config & Remote run snapshot with all inherited fields resolved. \\
...
\path{docs/713/HF_ARCHITECTURE_PROBE_2026-07-13.md} & diagnosis note & Human-readable interpretation of safe and unsafe target-HF routes. \\
\path{docs/713/REPO_REMOTE_AUDIT_2026-07-13.md} & audit note & Local/remote state audit used to separate paper evidence from scratch artifacts. \\
...
\midrule
Checkpoint directories under the remote experiment drive & \path{<EXP_ROOT>/<run_name>} & Checkpoints are portable; drive letters are not. \\
...
Remote shell commands and hostnames & omitted & They are operational details, not reproducibility requirements. \\
...
The formal supplement intentionally omits absolute local paths, SSH addresses, usernames,
and workstation drive letters. Those fields exist only in raw run artifacts and are not
