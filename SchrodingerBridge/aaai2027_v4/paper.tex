...
without an adversarial inner loop, denoising teacher, or inversion stage. Inference evaluates
the small network along a short path, applies one analytic transform, and decodes once.

Writing $C_v$ for one velocity evaluation and $C_{dec}$ for one frozen VAE decode,
the generation cost is approximately
\begin{equation}
...
Training-free diffusion editors avoid weight updates but repeatedly evaluate a large
denoiser, often with inversion. WEAVE combines VAE compression with cached encoding,
compact transport, a short path, closed-form alignment, and one decode; together these make
its training plus inference shorter than another method's inference alone.
...
the map only after the final Euler step keeps the image-specific correction out of later
velocity updates. LL anchoring protects layout, while the learned LH/HL path still carries
spatially organized edges and texture. The Latent-WCT control in
