...
LOG_DIR="${ROOT_DIR}/docs/experiments/phase2_fiber_bundle/616/logs/vertical_target_round1"
...
CONFIGS=(
  "configs/aaai2027/phase616_i2sb_vertical_target_a100_lowanchor050e9_b8a2_vlen010.json"
)
...
for config in "${CONFIGS[@]}"; do
  ...
  echo "[phase616_vertical_target_round1] starting ${config}" | tee "${log_path}"
  cmd=( "${PYTHON_BIN}" src/run.py --config "${config}" )
  if [[ -n "${RESUME_OVERRIDE}" ]]; then
    cmd+=( --resume "${RESUME_OVERRIDE}" )
  fi
  stdbuf -oL -eL "${cmd[@]}" > >(tee -a "${log_path}") 2> >(tee -a "${log_path}" >&2) &
  train_pid=$!
