...
echo "[phase616_idle_queue] start $(date -Iseconds) cwd=${ROOT_DIR}"
echo "[phase616_idle_queue] idle_mem_mib=${IDLE_MEM_MIB} poll_sec=${POLL_SEC}"

while true; do
  train_count="$(active_train_count)"
  used_mib="$(gpu_used_mib)"
  echo "[phase616_idle_queue] $(date -Iseconds) active_train=${train_count} gpu_used_mib=${used_mib}"
...
echo "[phase616_idle_queue] idle reached at $(date -Iseconds); launching auto tree"
exec bash tools/experiments/run_phase616_auto_tree.sh "$@"
