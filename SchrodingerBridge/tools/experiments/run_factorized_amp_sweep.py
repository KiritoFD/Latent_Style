...
DEFAULT_OUTPUT_ROOT = ROOT / "exp" / "factorized_amp_sweep"
DEFAULT_CONFIG_ROOT = ROOT / "configs" / "factorized_amp_sweep"
DEFAULT_EVAL_EPOCHS = (4, 6, 8)


@dataclass(frozen=True)
class FactorizedAmpCandidate:
    name: str
    enable_color: bool
    enable_amp: bool
...
def build_candidates() -> list[FactorizedAmpCandidate]:
    return [
        FactorizedAmpCandidate(name="f00_lowfreq_color_warp_ampoff", enable_color=True, enable_amp=False),
        FactorizedAmpCandidate(name="f01_lowfreq_color_amp_warp", enable_color=True, enable_amp=True),
        FactorizedAmpCandidate(name="f02_amp_warp_coloroff", enable_color=False, enable_amp=True),
    ]


def _config_payload(candidate: FactorizedAmpCandidate, *, base_config: Path, output_root: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "model": {
            ...
            "diffeomorphic_color_lowpass_kernel": candidate.color_lowpass_kernel,
            "diffeomorphic_color_edge_gamma": 0.0,
            "diffeomorphic_amp_strength": candidate.amp_strength,
            "diffeomorphic_factorized_enable_color": candidate.enable_color,
            "diffeomorphic_factorized_enable_amp": candidate.enable_amp,
        ...
        "ablation": {
            "name": candidate.name,
            "stage": "factorized_amp_sweep",
            "axis": "lowfreq_color_texture_amplitude_tangent_warp",
            "notes": "Replace additive high-frequency color residual with a multiplicative high-frequency amplitude sink.",
        },
    }
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Frequency-factorized amplitude head sweep.")
    ...
    ledger_path = args.output_root / "factorized_amp_ledger.jsonl"
    frontier_path = args.output_root / "factorized_amp_frontier.csv"
    ...
    for candidate in candidates:
        ...
        print(
            f"[candidate] color={candidate.enable_color} amp={candidate.enable_amp} "
            f"amp_strength={candidate.amp_strength} lp={candidate.color_lowpass_kernel}",
            flush=True,
        )
        ...
        row = {
            ...
            "enable_color": candidate.enable_color,
            "enable_amp": candidate.enable_amp,
            "amp_strength": candidate.amp_strength,
            "color_lowpass_kernel": candidate.color_lowpass_kernel,
