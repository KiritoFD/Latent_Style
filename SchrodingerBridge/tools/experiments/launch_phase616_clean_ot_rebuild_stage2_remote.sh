...
python tools/experiments/launch_remote_wsl_command.py \
  --task-name phase616_clean_ot_rebuild_stage2 \
  --remote-workspace-root /mnt/i/Github/Latent_Style \
  --remote-wsl-cwd /mnt/i/Github/Latent_Style/SchrodingerBridge \
  --remote-log-path /mnt/i/Github/Latent_Style/SchrodingerBridge/docs/experiments/phase2_fiber_bundle/616/logs/clean_ot_rebuild_stage2/launcher.log \
  --host 100.115.18.62 \
  --port 2222 \
  --user administrator \
  ...
  -- \
  bash tools/experiments/run_phase616_clean_ot_rebuild_stage2.sh
