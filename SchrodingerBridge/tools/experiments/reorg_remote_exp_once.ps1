...
foreach($d in $rootDirs){
  ...
  $moves += [pscustomobject]@{source=$d.FullName; destination=$dest; name=$d.Name; category= if($target.StartsWith('ARCHIVE:')){'archive'}else{($target -split '\\')[0]}; moved=$true}
...
foreach($f in $rootFiles){
  ...
  New-Item -ItemType Directory -Force -Path (Split-Path -Parent $dest) | Out-Null
  $moves += [pscustomobject]@{source=$f.FullName; destination=$dest; name=$f.Name; category='root_files'; moved=$true}
...
$manifestPath=Join-Path $manifestDir "remote_exp_reorg_$stamp.json"
...
[pscustomobject]@{manifest=$manifestPath; moved=$moves.Count; top=(Get-ChildItem -LiteralPath $exp -Directory | Select-Object -ExpandProperty Name)} | ConvertTo-Json -Compress
