...
BASE_CFG="${BASE_CFG:-$ROOT/docs/experiments/2026-06-18-stage1-lowrank-rerun-audit/baseline_h1_lowrank_config.json}"
STAGE_ROOT="${STAGE_ROOT:-$ROOT/exp/20250618_stage3_style_auto}"
REFERENCE_H1_DIR="${REFERENCE_H1_DIR:-$ROOT/exp/20250618_ot_rerun_lowrank_auto/h1_linear_fm}"
STYLE_SWEEP_FIXED_BATCH_SIZE="${STYLE_SWEEP_FIXED_BATCH_SIZE:-20}"
GENERATED_BASE_CFG="$STAGE_ROOT/_base_phase618_style_sweep_lowrank.json"
...
echo "============================================"
echo "  $(date) START phase618 style sweep"
echo "  base_cfg=$BASE_CFG"
echo "  generated_base_cfg=$GENERATED_BASE_CFG"
echo "  stage_root=$STAGE_ROOT"
echo "  reference_h1_dir=$REFERENCE_H1_DIR"
echo "  fixed_batch_size=$STYLE_SWEEP_FIXED_BATCH_SIZE"
echo "============================================"
...
if issues:
    raise SystemExit(
        "phase618 style-sweep base must already be the repaired lowrank carrier.\n- " + "\n- ".join(issues)
    )
...
python3 tools/experiments/phase616_auto.py style-sweep \
  --base-cfg "$GENERATED_BASE_CFG" \
  --stage-root "$STAGE_ROOT" \
  --reference-h1-dir "$REFERENCE_H1_DIR" \
  --skip-probe \
  --fixed-batch-size "$STYLE_SWEEP_FIXED_BATCH_SIZE" \
  "$@"

echo "============================================"
echo "  $(date) DONE phase618 style sweep"
echo "============================================"
