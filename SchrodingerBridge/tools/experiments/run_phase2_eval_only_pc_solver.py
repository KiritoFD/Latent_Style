...
DEFAULT_OVERRIDE = SB_ROOT / "configs" / "aaai2027" / "phase2_eval_pc_solver_xpred_pattn.json"


def _run(cmd: list[str]) -> int:
    print("[run_phase2_eval_only_pc_solver] " + " ".join(str(x) for x in cmd), flush=True)
    proc = subprocess.run(cmd, cwd=str(WORKSPACE), check=False)
    return int(proc.returncode)


def main() -> int:
    parser = argparse.ArgumentParser(description="Run eval-only phase2 solver_pc review on an existing checkpoint.")
    parser.add_argument("--checkpoint", required=True)
    parser.add_argument("--config-override", default=str(DEFAULT_OVERRIDE))
    ...
    output = (
        ...
        else checkpoint.parent / "full_eval_pc_solver" / checkpoint.stem
    )
