...
DEFAULT_OUTPUT_ROOT = ROOT / "exp" / "color_guard_sweep"
DEFAULT_CONFIG_ROOT = ROOT / "configs" / "color_guard_sweep"
DEFAULT_EVAL_EPOCHS = (4, 6, 8)


@dataclass(frozen=True)
class ColorGuardCandidate:
    name: str
    color_lowpass_kernel: int
    color_edge_gamma: float
...
def build_candidates() -> list[ColorGuardCandidate]:
    return [
        ColorGuardCandidate(name="cg01_t00_color_lp3", color_lowpass_kernel=3, color_edge_gamma=0.0),
        ColorGuardCandidate(name="cg02_t00_color_lp3_edge1p5", color_lowpass_kernel=3, color_edge_gamma=1.5),
        ColorGuardCandidate(name="cg03_t00_color_edge1p5", color_lowpass_kernel=1, color_edge_gamma=1.5),
        ColorGuardCandidate(name="cg04_t00_color_edge3p0", color_lowpass_kernel=1, color_edge_gamma=3.0),
    ]


def _config_payload(candidate: ColorGuardCandidate, *, base_config: Path, output_root: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "model": {
            ...
            "diffeomorphic_color_edge_gamma": candidate.color_edge_gamma,
        },
        "training": {
            "seed": candidate.seed,
        ...
        "ablation": {
            "name": candidate.name,
            "stage": "color_guard_sweep",
            "axis": "lowpass_and_edge_gated_color_residual",
            "notes": "Keep t00 tangent warp fixed and test whether color residual low-pass/edge gating improves content preservation.",
        },
    }
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Color residual guard sweep for tangent diffeomorphic stroke.")
    ...
    ledger_path = args.output_root / "color_guard_ledger.jsonl"
    frontier_path = args.output_root / "color_guard_frontier.csv"
    ...
    for candidate in candidates:
        ...
        print(
            f"[candidate] lp={candidate.color_lowpass_kernel} gamma={candidate.color_edge_gamma} "
        ...
        row = {
            ...
            "color_edge_gamma": candidate.color_edge_gamma,
