...
python3 tools/experiments/launch_remote_wsl_command.py \
  --task-name phase616_stage1_auto \
  --remote-workspace-root /mnt/i/Github/Latent_Style \
  --remote-wsl-cwd /mnt/i/Github/Latent_Style/SchrodingerBridge \
  --remote-log-path /mnt/i/Github/Latent_Style/SchrodingerBridge/docs/616/logs/phase616_stage1_auto.log \
  --host 100.115.18.62 \
  --port 2222 \
  --user administrator \
  ...
  -- \
  bash tools/experiments/run_phase616_stage1_auto.sh "$@"
