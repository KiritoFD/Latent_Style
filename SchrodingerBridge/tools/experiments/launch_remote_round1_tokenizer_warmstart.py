...
def _run(cmd: list[str]) -> int:
    print("[launch_remote_round1_tokenizer_warmstart] " + " ".join(str(x) for x in cmd), flush=True)
    proc = subprocess.run(cmd, cwd=str(WORKSPACE), check=False)
    return int(proc.returncode)


def main() -> int:
    parser = argparse.ArgumentParser(description="Prepare and optionally launch a round-1 tokenizer warm-start packet.")
    ...
    prepare = [
        sys.executable,
        str(SCRIPT_DIR / "prepare_round1_tokenizer_warmstart_config.py"),
        "--family-id",
        str(args.family_id),
        "--manifest-csv",
    ...
    if str(args.teacher_checkpoint).strip():
        prepare.extend(["--teacher-checkpoint", str(args.teacher_checkpoint)])
    ...
    for line in prep_proc.stdout.splitlines():
        ...
        if text.endswith(".json"):
            warmstart_config = text
            break
    if not warmstart_config:
        raise RuntimeError("prepare_round1_tokenizer_warmstart_config.py did not report a warmstart config path")
    ...
    launch = [
        ...
        "--config",
        str(Path(warmstart_config).resolve().relative_to(WORKSPACE.resolve())),
    ]
