...
DEFAULT_LOCAL_ROOT = SB_ROOT / "aaai2027" / "dualpath_spatial_sinkhorn_bestfew_localreview_20260609"


def _run(cmd: list[str]) -> int:
    print("[pull_remote_dualpath_spatial_sinkhorn_bestfew_localreview] " + " ".join(str(x) for x in cmd), flush=True)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Pull remote dualpath spatial sinkhorn eval curve and best-few epoch dirs to local storage.")
    ...
    for row in rows:
        ...
        rc = _run(
            [
                ...
                "--remote-dir",
                f"{REMOTE_RUN_DIR}/{eval_subdir}/{epoch}",
                "--local-dir",
                str(local_eval_root / epoch),
                "--tar-name",
                f"{eval_subdir}_{epoch}.tar",
            ]
        )
        if rc != 0:
            return rc
