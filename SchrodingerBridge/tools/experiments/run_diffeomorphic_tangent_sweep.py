...
DEFAULT_OUTPUT_ROOT = ROOT / "exp" / "diffeomorphic_tangent_sweep"
DEFAULT_CONFIG_ROOT = ROOT / "configs" / "diffeomorphic_tangent_sweep"
...
def _config_payload(candidate: TangentCandidate, *, base_config: Path, output_root: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "ablation": {
            "name": candidate.name,
            "stage": "diffeomorphic_tangent_sweep",
            "axis": "texture_tangent_warp_grid",
            "notes": "Sweep warp strength, texture gate, and normal leak around the best tangent-warp branch.",
        },
    }
...
def build_candidates() -> list[TangentCandidate]:
    ...
    grid_warp = [0.03, 0.05, 0.07]
    grid_gate = [6.0, 8.0]
    grid_leak = [0.0, 0.05]
    for idx, (warp_strength, gate_strength, normal_leak) in enumerate(
        [(w, g, l) for w in grid_warp for g in grid_gate for l in grid_leak]
    ):
        name = f"t{idx:02d}_ws{_fmt(warp_strength)}_g{_fmt(gate_strength)}_nl{_fmt(normal_leak)}"
        candidates.append(
            TangentCandidate(
                name=name,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="12-run tangent-warp ablation sweep.")
    ...
    ledger_path = args.output_root / "tangent_grid_ledger.jsonl"
    frontier_path = args.output_root / "tangent_grid_frontier.csv"
    mem_gb = _detect_gpu_memory_gb()
    default_batch_size, default_eval_batch_size = _tiered_batch_sizes()
    ...
    for candidate in candidates:
        ...
        print(
            ...
            f"leak={candidate.normal_leak} color={candidate.color_strength}",
            flush=True,
        )
