...
python tools/experiments/launch_remote_wsl_command.py \
  --task-name phase616_clean_ot_probe_round6_featurehybrid \
  ...
  --remote-log-path /home/xy/Latent_Style/SchrodingerBridge_phase616/SchrodingerBridge/docs/experiments/phase2_fiber_bundle/616/logs/clean_ot_probe_round6_featurehybrid/launcher.log \
  --host 100.115.18.62 \
  --port 2222 \
  --user administrator \
  ...
  -- \
  bash tools/experiments/run_phase616_clean_ot_probe_round6_featurehybrid.sh
