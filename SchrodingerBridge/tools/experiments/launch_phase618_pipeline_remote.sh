...
python3 tools/experiments/launch_remote_wsl_command.py \
  --task-name phase618_pipeline \
  --remote-workspace-root /mnt/i/Github/Latent_Style \
  --remote-wsl-cwd /mnt/i/Github/Latent_Style/SchrodingerBridge \
  --remote-log-path /mnt/i/Github/Latent_Style/SchrodingerBridge/docs/616/logs/phase618_pipeline.log \
  --host 100.115.18.62 \
  --port 2222 \
  --user administrator \
  ...
  --sync-path SchrodingerBridge/docs/experiments/2026-06-18-stage1-lowrank-rerun-audit/remote_base_phase618_ot_rerun_lowrank.json \
  ...
  -- \
  bash tools/experiments/run_phase618_pipeline.sh
