...
python tools/experiments/launch_remote_wsl_command.py \
  --task-name phase616_clean_stats_probe_round2_authoritative \
  ...
  --remote-log-path /home/xy/Latent_Style/SchrodingerBridge_phase616/SchrodingerBridge/docs/experiments/phase2_fiber_bundle/616/logs/clean_stats_probe_round2_authoritative/launcher.log \
  --host 100.115.18.62 \
  --port 2222 \
  --user administrator \
  ...
  --verify-python-file SchrodingerBridge/src/model.py \
  ...
  --verify-python-file SchrodingerBridge/tools/experiments/build_phase616_style_stats_bank.py \
  ...
  -- \
  bash tools/experiments/run_phase616_clean_stats_probe_round2_authoritative.sh
