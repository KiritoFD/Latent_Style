...
SEGMENTED_SCRIPT = SCRIPT_DIR / "run_remote_round1_family_segmented.py"
...
def _stop_existing_segmented_controllers(*, family_id: str) -> None:
    pids = _find_local_python_pids_for_tokens("run_remote_round1_family_segmented.py", f"--family-id {family_id}")
    if not pids:
        return
    subprocess.run(
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Launch run_remote_round1_family_segmented.py as a detached local background controller.")
