...
"""SD 1.5 adaptation of the StyleAligned attention handler."""
...
def expand_first(feat: T, scale: float = 1.0) -> T:
    b = feat.shape[0]
...
def concat_first(feat: T, dim: int = 2, scale: float = 1.0) -> T:
    feat_style = expand_first(feat, scale=scale)
    return torch.cat((feat, feat_style), dim=dim)


def calc_mean_std(feat: T, eps: float = 1e-5) -> tuple[T, T]:
...
class DefaultAttentionProcessor(nn.Module):
    ...
    def __call__(
        ...
        hidden_states: T,
        encoder_hidden_states: T | None = None,
        attention_mask: T | None = None,
        **kwargs,
    ) -> T:
...
class SharedAttentionProcessor(DefaultAttentionProcessor):
    ...
    def __init__(self, style_aligned_args: StyleAlignedArgs) -> None:
        super().__init__()
...
def _get_switch_vec(total_num_layers: int, level: float) -> T:
    if level == 0:
...
def init_attention_processors(
    pipeline: StableDiffusionPipeline, style_aligned_args: StyleAlignedArgs | None = None
) -> None:
...
def register_shared_norm(
    ...
    def register_norm_forward(norm_layer: nn.Module) -> nn.Module:
    ...
    def get_norm_layers(pipeline_module: nn.Module, norm_layers_: dict[str, list[nn.Module]]) -> None:
        if isinstance(pipeline_module, nn.LayerNorm) and share_layer_norm:
            norm_layers_["layer"].append(pipeline_module)
        if isinstance(pipeline_module, nn.GroupNorm) and share_group_norm:
            norm_layers_["group"].append(pipeline_module)
        for layer in pipeline_module.children():
            get_norm_layers(layer, norm_layers_)

    norm_layers: dict[str, list[nn.Module]] = {"group": [], "layer": []}
...
class Handler:
    ...
    def register(self, style_aligned_args: StyleAlignedArgs) -> None:
        self.norm_layers = register_shared_norm(
            self.pipeline, style_aligned_args.share_group_norm, style_aligned_args.share_layer_norm
        )
