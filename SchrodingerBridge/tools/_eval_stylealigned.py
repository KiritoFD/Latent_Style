"""StyleAligned baseline evaluation: CLIP-S, LPIPS, MUSIQ on D5-512 and P2A-256."""
...
def _get_image_features(clip_model, clip_proc, img):
    """Extract CLIP image features as numpy."""
...
def compute_lpips(gen_paths, src_paths, lpips_metric):
    """Mean LPIPS between gen and source (resized to match)."""
    vals = []
    for gp, sp in zip(gen_paths, src_paths):
        gen = Image.open(gp).convert("RGB")
        src = Image.open(sp).convert("RGB")
        # Resize both to the generated image size to ensure matching dims
        w, h = gen.size
...
def _find_source_file(src_dirs, src_stem):
    """Search multiple directories for a source file by stem."""
...
def collect_pairs_d5(img_dir, test_dir, train_dir, styles):
    """Collect (gen_path, src_path, tgt_style) tuples for D5-512 dataset.

    Generated filename: Style__Style__artist_title__to__TargetStyle.png
    Source file:        [train|test]/Style/Style__artist_title.jpg
    """
    ...
    for gen_file in sorted(img_dir.glob("*.png")):
        stem = gen_file.stem  # e.g. Impressionism__Impressionism__artist_title__to__Ukiyo_e
        if "__to__" not in stem:
            continue
        left, tgt_style = stem.rsplit("__to__", 1)
        # left = Style__Style__artist_title
        parts = left.split("__", 2)
        if len(parts) < 3:
            continue
        src_style = parts[0]
        src_stem = parts[0] + "__" + parts[2]  # Style__artist_title
        # Search both train and test for source
...
def collect_pairs_p2a(img_dir, test_dir, styles):
    """Collect (gen_path, src_path, tgt_style) tuples for P2A-256 dataset.

    Generated filename: Style__number__to__TargetStyle.png  (e.g. cezanne__00605__to__vangogh.png)
    Source test file:   test/Style/number.jpg                (e.g. test/cezanne/00605.jpg)
    """
    img_dir = Path(img_dir)
    test_dir = Path(test_dir)
    pairs = []
...
def eval_dataset(pairs, test_dir, styles, label):
    """Evaluate CLIP-S, LPIPS, MUSIQ for a dataset."""
    test_dir = Path(test_dir)
...
def main():
    print("=" * 60)
    print("StyleAligned Baseline Evaluation")
    print("=" * 60)

    # ---- P2A-256 ----
    p2a_styles = ["cezanne", "Hayao", "monet", "photo", "vangogh"]
    p2a_pairs = collect_pairs_p2a(
        r"g:\GitHub\Latent_Style\SchrodingerBridge\results\P256\stylealigned",
    ...
    d5_pairs = collect_pairs_d5(
        r"g:\GitHub\Latent_Style\SchrodingerBridge\results\D5-512\stylealigned",
    ...
    # ---- R5-512 (Random5 hold-out styles) ----
    ...
    r5_pairs = collect_pairs_d5(
        r"g:\GitHub\Latent_Style\SchrodingerBridge\results\R5-512\stylealigned",
        rf"{r5_base}\test",
        rf"{r5_base}\train",
        r5_styles,
    )
    ...
    print("=== FINAL RESULTS ===")
    if mq1 is not None:
        print(f"P2A-256 StyleAligned:  CLIP-S={cs1:.4f}  LPIPS={lp1:.4f}  MUSIQ={mq1:.4f}")
    else:
        print(f"P2A-256 StyleAligned:  CLIP-S={cs1:.4f}  LPIPS={lp1:.4f}")
    if mq2 is not None:
        print(f"D5-512  StyleAligned:  CLIP-S={cs2:.4f}  LPIPS={lp2:.4f}  MUSIQ={mq2:.4f}")
    else:
        print(f"D5-512  StyleAligned:  CLIP-S={cs2:.4f}  LPIPS={lp2:.4f}")
    if mq3 is not None:
        print(f"R5-512  StyleAligned:  CLIP-S={cs3:.4f}  LPIPS={lp3:.4f}  MUSIQ={mq3:.4f}")
    else:
        print(f"R5-512  StyleAligned:  CLIP-S={cs3:.4f}  LPIPS={lp3:.4f}")
    print("=" * 60)
