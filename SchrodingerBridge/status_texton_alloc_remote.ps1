...
$root = 'exp\vae_backend\ema_transport_texton_alloc'
...
"--- numeric debug allocation tail ---"
$debugs = Get-ChildItem $root -Recurse -Filter numeric_debug.jsonl -ErrorAction SilentlyContinue |
    Sort-Object LastWriteTime -Descending |
    Select-Object -First 3
foreach ($debug in $debugs) {
    "debug: $($debug.FullName)"
    Get-Content $debug.FullName -Tail 2
}
