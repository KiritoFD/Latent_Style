...
On Distinct5-WikiArt, the 903K-parameter WD-VF model reaches CLIP-S\,=\,0.7213 and LPIPS\,=\,0.2868.
...
WD-VF solves the problem by changing coordinates rather than enlarging the generator.
...
(4)~We show that this geometry gives an efficient real-transfer regime. WD-VF uses 903K parameters and trains in 3.08 minutes. Crucially, the decoupled transport is highly extensible: we demonstrate that the model can generalize to unseen styles via lightweight style-memory updating, eliminating the need for full retraining.
...
\tfrac12(w_f\mu-w_s\nu)\|\delta\|^2 + o(\|\delta\|^2).
\end{equation}
The coefficient of $\|\delta\|^2$ is positive. Therefore $\Delta \mathcal L_{\mathrm{euc}}(\delta)>0$ for all sufficiently small nonzero $\delta$.
\end{proof}
...
Theorem~\ref{thm:attractor} is the first half of the paper's causal chain.
It identifies a concrete Euclidean failure regime and predicts what should happen when that regime is broken.
If we remove the shared Euclidean coordinates, the CLIP-S/LPIPS frontier should improve.
If a Euclidean fit remains trapped near $\mathcal M_0$, its retained style amplitude should stay small.
In our Euclidean fit, the amplitude estimate is $\kappa\approx0.016$.
In Table~\ref{tab:ablation}, removing the wavelet coordinate change weakens the frontier exactly as predicted.

The theorem is local rather than global.
It does not claim that every Euclidean flow must collapse.
It states that near $\mathcal M_0$, flow matching can dominate endpoint style supervision in the normal directions.
That is precisely the regime relevant to our benchmark: the Euclidean baseline has low LPIPS, sub-IDT CLIP-S, and a fitted style amplitude close to zero.
...
\end{proposition}
\begin{proof}
Lemma~\ref{lem:iso} removes all cross terms between $\ell$ and $h$.
The only content perturbation left is the small content residue already present in $h$.
\end{proof}

Proposition~\ref{prop:decouple} states the geometric premise of WD-VF.
When most content energy lies in LL, high-frequency transport changes texture much more than layout.
This motivates weak LL supervision and high-frequency routing.

This premise is approximate rather than absolute.
High-frequency bands can still contain some content residue, and some styles also change coarse palette.
WD-VF therefore keeps a weak LL term instead of setting it to zero in the final model.
The wavelet basis is useful because it makes the dominant conflict explicit and controllable.
...
Proposition~\ref{thm:blockdiag} identifies the active supervised terms in the routed limit and turns that structure into ablation-level predictions.
Train-time routing should matter because inference uses the high-frequency path.
Repeated style injection should be unnecessary once that path is learned.
Endpoint-only alignment should therefore preserve LPIPS better than per-step injection.
Tables~\ref{tab:ablation} and~\ref{tab:theorylink} test these predictions directly.
...
\end{proposition}
\begin{proof}
Each map $T_i$ acts only on $h_i$. The first coordinate of $\mathcal W(\hat z_1^+)$ is therefore still $\hat \ell$. The inverse wavelet transform preserves that base coordinate.
\end{proof}
...
\end{proposition}
\begin{proof}
Each injection preserves a fraction $1-\alpha$ of the current content residue. Composing $n$ identical injections gives $r_n(\alpha)=(1-\alpha)^n$. The case $n=1$ is affine in $\alpha$. By contrast, $n\gg1$ compresses the same control range exponentially.
\end{proof}

Proposition~\ref{prop:eota} gives the third link in the theory-to-experiment chain.
If style is injected at every solver step, the residual content fraction shrinks exponentially.
The prediction is simple: repeated injection should raise LPIPS without being necessary for strong transfer.
Table~\ref{tab:ablation} confirms this pattern.
Per-step AdaIN increases CLIP-S but raises LPIPS sharply, whereas endpoint-only alignment preserves a much better frontier.

\subsection{Architecture and Measured Cost}
\label{sec:arch}

The final model uses four residual blocks at width $64$, three $3\times3$ velocity heads (LL/LH/HL), and 903K trainable parameters.
...
Relative to Seedream 4.5, it stays close in CLIP-S and reduces LPIPS from 0.4767 to 0.2868, a 39.8\% reduction from a 903K local model.
...
On Distinct5-WikiArt, it reaches CLIP-S/LPIPS of 0.7213/0.2868 with 903K parameters.
Training takes 3.08 minutes on a single RTX 3060.
...
The 903K-parameter checkpoint is 3.5\,MB.
