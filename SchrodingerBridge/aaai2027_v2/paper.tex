...
SaMam~\cite{liu2024samam} and SaMST~\cite{liu2024samst} adapt the Mamba state-space model~\cite{gu2023mamba} to style encoding. SaMam's reported CLIP-S (0.5816) falls below the identity baseline (0.6933), indicating content reconstruction rather than style transfer; its low LPIPS (0.2434) is therefore a style-failure symptom, not a content-preservation win. Our method uses a lightweight per-domain token bank instead of a Mamba encoder, and trains $141\times$ faster.
...
All experiments use public data (WikiArt) and a public VAE (SDXL VAE). The training configuration, hyperparameters, and evaluation protocol are described in Section~\ref{sec:exp} and the supplement. Training takes 3 minutes on a single RTX 3060 (12\,GB); evaluation takes 97 seconds on the same hardware. The 903K-parameter checkpoint is 3.5\,MB. We will release the code, the checkpoint, and the 750-pair evaluation protocol upon publication. The 645-configuration ablation audit is documented in the supplement.

\section*{AAAI Reproducibility Checklist}

\begin{enumerate}
\item This paper claims the following contributions: (1) diagnosis of the Pareto deadlock and degeneration attractor (Section~\ref{sec:attractor}); (2) Spectral Flow Matching method (Section~\ref{sec:method}); (3) experimental validation on 750 pairs (Section~\ref{sec:exp}); (4) 645-configuration subtractive ablation (Section~\ref{sec:ablation}).
...
\item The 645-configuration ablation audit (Section~\ref{sec:ablation}, Table~\ref{tab:ablation}) is documented in the supplement.
\item Statistical significance: all reported numbers are single-seed; multi-seed variance is below 0.002 CLIP-S for SFM (see supplement).
\end{enumerate}
