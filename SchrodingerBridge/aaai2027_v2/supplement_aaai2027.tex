...
Table~\ref{tab:matrix_supp} reproduces the full $5\times5$ transfer matrix of the main paper (Table~5). Diagonal entries are identity (no transfer); off-diagonals are cross-domain transfers.

\begin{table}[h]
\centering
\caption{$5\times5$ transfer matrix: CLIP-S (left) and LPIPS (right) for every (source, target) pair. ER=Early Renaissance, Imp=Impressionism, Min=Minimalism, Roc=Rococo, Uki=Ukiyo-e.}
\label{tab:matrix_supp}
\footnotesize
\setlength{\tabcolsep}{4pt}
