...
class OTFlowMatchingObjective:
    def __init__(self, config: Dict) -> None:
        ...
        self.color_transport_mode = str(bridge_cfg.get("color_transport_mode", "softmax")).strip().lower()
        if self.color_transport_mode not in {"softmax", "gumbel_hard"}:
            self.color_transport_mode = "softmax"
        self.color_gumbel_tau = max(1e-3, float(bridge_cfg.get("color_gumbel_tau", 1.0)))
