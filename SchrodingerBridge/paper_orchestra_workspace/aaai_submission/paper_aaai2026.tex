...
\begin{itemize}
    ...
    \item We provide a strict 750-output evaluation protocol with complete local core-metric comparisons to AdaIN, S2WAT, SaMST, StyleID, StyTr2, CAST, AesFA, AesPA-Net, and our method, together with artifact-sensitive diagnostics for grain and perceptual quality.
...
\begin{tabular}{@{}p{0.31\linewidth}p{0.31\linewidth}p{0.27\linewidth}@{}}
\toprule
Method & Status & Role \\
\midrule
...
StyTr2 & complete strict-750 core metrics + efficiency & transformer AST baseline \\
CAST & complete strict-750 core metrics + efficiency & training-free baseline \\
AesFA & complete strict-750 core metrics + efficiency & aesthetic arbitrary-style baseline \\
AesPA-Net & complete strict-750 core metrics + efficiency & pattern-aware arbitrary-style baseline \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:baseline_status} is included to avoid overstating the comparison set. All methods listed here now have complete strict-750 generations. For StyTr2, CAST, AesFA, and AesPA-Net we additionally report unified core metrics (CLIP-style, CLIP-content, LPIPS, and EC) and hardware-aligned efficiency measurements. The full artifact pack remains concentrated on Ours versus SaMST, where the visible muddy/grain failure mode is most pronounced.
...
Table~\ref{tab:main} shows that Ours epoch 7 remains close to SaMST in raw CLIP-style while improving LPIPS and EC. StyleID obtains the highest CLIP-style, but its CLIP-content of 0.5519 and LPIPS of 0.7497 indicate severe semantic drift. The newly completed StyTr2, CAST, AesFA, and AesPA-Net runs are all clearly below the main frontier under the strict-750 protocol: their CLIP-style remains substantially lower than Ours/SaMST/StyleID, and their LPIPS ranges from 0.6345 to 0.8215. These added baselines therefore strengthen, rather than weaken, the main empirical positioning of LBM as a competitive content-preserving trade-off point.
...
The experiments were run from a reproducible workspace with saved configs, checkpoints, strict-750 outputs, metrics, and logs. The primary configuration is stored in the K1-SWD20 run directory. Destructive ablations, the 40-run weight sweep, the review-added step sweep, and the $\lambda$-grid validation are stored in separate experiment directories with their own configs and training logs. For efficiency, we now distinguish two measurement scopes. The first is an end-to-end strict-750 measurement that includes generation, decoding, file I/O, and protocol packaging. The second is a unified forward micro-benchmark that profiles the stylization network itself under a fixed local hardware/software stack. This separation is important because file-system overhead can partially mask step-count differences in end-to-end runs.
...
\begin{table}[t]
\centering
\caption{Unified local efficiency profiling. ``Prof img/s'' measures latent-generator throughput under a common micro-benchmark, whereas ``E2E img/s'' measures the full strict-750 inference path including file I/O and decoding.}
\label{tab:runtime}
\resizebox{\columnwidth}{!}{%
