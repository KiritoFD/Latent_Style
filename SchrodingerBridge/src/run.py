...
def main() -> None:
    ...
    while epoch <= int(trainer.num_epochs):
        ...
        logger.info(
            "Epoch %d/%d | loss=%.4f flow=%.4f kin=%.4f ot=%.4f tswd=%.4f sem_attn=%.3f sem_k=%.3f plan_ent=%.3f tok_ent=%.3f tok_max=%.3f sigma=%.3f idr=%.2f t=%.3f |v|=%.3f lr=%.2e data=%.1fs comp=%.1fs peak=%.2f/%.2fGB",
            epoch,
            trainer.num_epochs,
