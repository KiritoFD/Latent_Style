...
def generate_summary_json(
    ...
    *,
    ...
    identity_pool = []
    photo_transfer_pool = []
    target_style_pool = defaultdict(list)
    target_style_transfer_pool = defaultdict(list)
    source_style_pool = defaultdict(list)
    source_style_transfer_pool = defaultdict(list)
    ...
    for src, targets in matrix.items():
        ...
        for tgt, items in targets.items():
            ...
            target_style_pool[str(tgt)].append(stats)
            source_style_pool[str(src)].append(stats)
            
            if src == tgt:
                identity_pool.append(stats)
            else:
                transfer_pool.append(stats)
                target_style_transfer_pool[str(tgt)].append(stats)
                source_style_transfer_pool[str(src)].append(stats)
                if src == 'photo':
    ...
    def pool_count(pool) -> int:
        return int(np.sum([int(x.get("count", 0) or 0) for x in pool])) if pool else 0
    ...
    def build_pool_summary(pool, *, valid: bool | None = None):
        return {
            'pair_count': int(len(pool)),
            'image_count': pool_count(pool),
    ...
    def build_group_summary(grouped_pools):
        return {
            str(name): build_pool_summary(pool, valid=len(pool) > 0)
            for name, pool in sorted(grouped_pools.items(), key=lambda item: str(item[0]).lower())
        }
    ...
    summary = {
        ...
        'analysis': {
            ...
            'by_target_style': build_group_summary(target_style_pool),
            'cross_by_target_style': build_group_summary(target_style_transfer_pool),
            'by_source_style': build_group_summary(source_style_pool),
            'cross_by_source_style': build_group_summary(source_style_transfer_pool),
        },
