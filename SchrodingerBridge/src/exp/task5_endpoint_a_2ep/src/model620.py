...
class FiLMEndpointHead(nn.Module):
    ...
    def __init__(self, dim: int, latent_channels: int, style_dim: int, style_hidden_dim: int, film_init_std: float = 0.0, use_norm: bool = True) -> None:
        super().__init__()
        ...
        if self.endpoint_head_mode == "endpoint_lowhigh":
            ...
            if self.endpoint_film_enabled:
                self.endpoint_film_low = FiLMEndpointHead(
                    self.dim, self.latent_channels, self.dim, self.endpoint_style_hidden_dim, self.endpoint_film_init_std, self.endpoint_film_use_norm,
                )
