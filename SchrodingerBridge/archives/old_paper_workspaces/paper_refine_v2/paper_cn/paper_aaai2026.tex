...
The components address distinct subproblems. OT coupling provides target-style endpoints for unpaired supervision, but not transport dynamics. Flow matching learns how to move toward those endpoints, but by itself does not guarantee that the integrated endpoint matches target-domain texture statistics. Terminal SWD regularizes this endpoint mismatch, while semantic cross-attention provides local routing for where style is written. The destructive ablations directly verify the roles of terminal SWD and kinetic regularization; additional component studies isolating OT coupling and semantic routing are left for future work.
...
\toprule
Method & Params & FLOPs(G) & CLIP-S$\uparrow$ & LPIPS$\downarrow$ & EC$\uparrow$ & Infer & s/img & Train sec \\
\midrule
Ours ep7 & 3.9 M & 5.4 & 0.716 & 0.451 & 0.393 & 40.0 s & 0.053 & 309.9 \\
...
\smallskip
\footnoterule\small
$\dagger$Extrapolated. FLOPs estimated via single forward pass; Ours uses 5.4~GFLOPs for the 1-step endpoint mapping.
\end{table*}
...
Against S2WAT, LBM is not only far smaller and cheaper to train, but also stronger on the core transfer metrics: higher CLIP-style, substantially lower LPIPS, and higher EC. This comparison shows that the efficiency gain is not obtained by sacrificing transfer quality---LBM is clearly Pareto-favorable over this heavier baseline.

AdaIN remains faster at inference but lies clearly below the high-quality frontier (LPIPS 0.630, EC 0.264), while StyleID attains the highest raw CLIP-style (0.760) at the cost of severe content drift (LPIPS 0.750, CLIP-content 0.552) and much slower inference (603 s per 750 images). CAST also trails on core metrics. Additional arbitrary-style baselines (StyTr2, AesFA, AesPA-Net) reported in prior work \cite{deng2022stytr2,huang2024aesfa,hong2023aespa} fall clearly below the frontier under matched evaluation but are omitted from Table~\ref{tab:main} because their training-time measurements were unavailable from our protocol. Taken together, these comparisons show that LBM is competitive both as an efficient replacement for the strongest lightweight baseline (SaMST) and as a stronger alternative to a heavier transformer-based transfer model (S2WAT).
