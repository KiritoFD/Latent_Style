...
\begin{table}[t]
\centering
\caption{Unified efficiency profiling. ``Prof img/s'' measures latent-generator throughput under a common micro-benchmark, whereas ``E2E img/s'' measures the full strict-750 inference path including file I/O and decoding. Training times marked with $\dagger$ are extrapolated from a single-epoch or single-iteration probe multiplied by the total training length.}
\label{tab:runtime}
\resizebox{\columnwidth}{!}{%
