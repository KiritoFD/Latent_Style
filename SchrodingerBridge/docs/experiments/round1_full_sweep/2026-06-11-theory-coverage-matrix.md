...
| Predictor-corrector solver | `solver_pc` | implemented | `running` | current formal active lane; all-ckpt authority settled through `epoch_0024` |
...
|---|---|---|---|
| Active formal lane | `solver_pc` | `running` | best transfer style still `epoch_0001`, best LPIPS still `epoch_0009`, newest settled point `epoch_0024`, past patience but still `tail_flat=false` |
