...
> FC-SB (Frequency-Conditioned Schrödinger Bridge) 的完整理论体系：从 Schrödinger Bridge 的一般框架，到 Flow Matching 训练目标，到 Haar DWT 频域解耦，再到 EOTA 与 DWT Route 的设计原理。
...
给定源域 content latent `x_0 ∈ R^{4×32×32}`（SDXL VAE 编码）与目标风格 `s ∈ {1,…,5}`，生成 `x_1` 使其：
...
| `transfer_clip/lpips` | 对角线（src=tgt）自转移 | 单风格保真度参考 |

### 1.3 双超目标

`all_pairs_clip > 0.7319 AND all_pairs_lpips < 0.3068`

- 0.7319 = 远程 4F.1 SOTA clip
- 0.3068 = 远程 4J.1 DWT route lpips
...
**4I.10 Probe 诊断**：velocity field 在 t=0.5（轨迹中点）几乎完全死亡（cos similarity ≈ 0.01）。FM 的固有歧义：中点方向不确定，因为 `x_0.5 = 0.5·content + 0.5·target`，模型无法从混合状态恢复方向。**模型本质是端点校正器，而非 ODE**。
...
其中 `a,b,c,d` 是 2×2 patch 的 4 个像素。
...
这是 per-subband AdaIN/WCT 统计隔离的理论基础：在 DWT 域对各子带独立做统计匹配，等价于在原域做不同频段的独立风格注入。

### 3.3 多级分解

多级 DWT 递归地对 LL 子带再分解：
...
| 子带 | 信息 | 风格贡献 | 内容贡献 |
|------|------|----------|----------|
| **LL** | 全局色调、光照、色相、低频结构 | **高**（+0.014 clip，4G.1a vs 4G.1b） | **高**（lpips 锚） |
...
628 L8 实验：训练 `head_hh` 与不训练 clip 差异 Δ=±0.0001（DEAD）。HH velocity head 在 SOTA 配置中删除，模型输出仅 `{ll, lh, hl}`。这是模型简化的关键决策。
...
**Cross-Attention Gate**：`style_cross_attn_gate_init=0.05`，输出经 `tanh` 门控后残差注入，避免训练初期破坏内容。
...
**T5/T10/T11/T12 共 15 个配置**系统性证明：在当前 DWT route 架构下，风格注入和内容保护存在固有的 1:8 trade-off（clip 每提升 1 单位，lpips 损失 8 单位）。要达成双超目标需要 trade-off 比 ≤ 1:1.9，**架构上不可达**。
...
**T10 (p=0.5) 失败根因**：50% 概率看 DWT 系数仍不足以让 q_proj 精通 DWT 分布。q_proj 倾向于"平均"两种分布，导致两种都处理不好（clip=0.7083 FAIL）。
...
- clip=0.7213（PASS 0.7243 阈值附近，但差目标 0.7319 共 0.0106）
...
**T19a 数值稳定性修复**：depth=6 时协方差矩阵病态，`eigh` 分解失败。修复方案：
- 对角线正则化 `Σ + eps·I`
- try-except 回退到 AdaIN
