...
3. The current Distinct5 frontier claim is directly supported at the paper-safe level already used in the tex: among the currently reproduced positive-`Delta_idt` points, LBM-F and LBM-K define the strongest measured `CLIP-S / LPIPS` trade-off. SaMAM's supported failure mode is target-direction failure: its measured transfer-only `CLIP-S` remains below the `idt` floor, and LPIPS is not the rejection criterion. SaMST reaches higher raw style only in a severe LPIPS / ArtFID regime.
