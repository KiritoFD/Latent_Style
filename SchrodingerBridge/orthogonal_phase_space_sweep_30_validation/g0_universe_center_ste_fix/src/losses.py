...
def calc_latent_patch_nce_loss(
    ...
    *,
    ...
    q_norm = F.normalize(q_sampled.float(), p=2, dim=-1)
    k_norm = F.normalize(k_sampled.float(), p=2, dim=-1)
...
class OTFlowMatchingObjective:
    ...
    def _calc_local_contextual_color_loss(
        ...
        *,
        ...
        attn = F.softmax(attn / 0.07, dim=-1)
    ...
    def _semantic_guided_swd(
        ...
        theta = F.normalize(semantic_axes, p=2, dim=1)
    ...
    def _collect_repulsive_components(
        ...
        *,
        ...
        feats = F.normalize(pooled.flatten(1), dim=1)
    ...
    def _compute_omf(
        ...
        *,
        ...
        # Straight-through hard semantic routing: use argmax in the forward pass
        # to keep patch assignments crisp, but preserve the soft attention graph
        # for backprop so Q/K can still learn meaningful correspondences.
        if attn_plan is not None:
            b_attn, num_q, num_k = attn_plan.shape
            idx = attn_plan.argmax(dim=-1)
            hard_attn = F.one_hot(idx, num_classes=num_k).to(dtype=attn_plan.dtype)
            ste_attn = hard_attn.detach() - attn_plan.detach() + attn_plan

            b_tgt, c_tgt, h_tgt, w_tgt = target_style.shape
            if b_tgt != b_attn or (h_tgt * w_tgt) != num_k or num_q != num_k:
                pseudo_target = target_style
            else:
                target_flat = target_style.reshape(b_tgt, c_tgt, -1).transpose(1, 2)
                pseudo_target = torch.bmm(ste_attn, target_flat).transpose(1, 2).reshape(b_tgt, c_tgt, h_tgt, w_tgt)

            pseudo_tgt_low, pseudo_tgt_high = self._freq_split(
                pseudo_target,
                kernel_size=self.low_freq_kernel_size,
            )
        else:
            pseudo_tgt_low, pseudo_tgt_high = self._freq_split(
                target_style,
        ...
        if self.w_low_freq > 0.0:
            ...
            # we anchor them to the semantically reassembled target rather than
            # the source content, keeping the edit path alive.
            low_freq_loss = F.l1_loss(pred_low, pseudo_tgt_low)
        ...
        if self.terminal_swd_weight > 0.0:
            swd_loss = self.transport_cost.aligned_cost(pred_high, pseudo_tgt_high)
        else:
