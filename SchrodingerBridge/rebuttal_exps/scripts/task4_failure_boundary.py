"""Task 4: Failure Boundary Mapping via orthogonal hyperparameter sweep.

Goal: Precisely map the (lambda_LL, eta) coordinates where the WEAVE transport
field tears, proving the production hyperparameters sit inside the unique
non-collapse topological manifold, not merely an empirical choice.

Sweep grid:
  lambda_LL (endpoint_adain_scale_ll) in {0.0, 0.1, 0.3, 1.0, 3.0, 10.0}
  eta       (style_extrap_alpha)      in {0.0, 0.1, 0.5, 1.0, 2.0}
  -> 6 x 5 = 30 grid points

Per grid point:
  - 5 source/target pairs (same as Task 2)
  - Compute DINO-C (content preservation) and LPIPS
  - Catch exceptions (gradient explosion / NaN) -> mark as crashed

Production point: (lambda_LL=1.0, eta=0.1)
  Note: endpoint_adain_scale_ll=-1.0 in checkpoint falls back to
  endpoint_adain_scale=1.0, so the effective production lambda_LL is 1.0.

Failure criteria:
  DINO-C < 0.215 (TGT DINO-C floor) OR forward dynamics gradient explosion.

Output:
  task4_grid_results.json   - full per-pair results
  task4_grid_summary.csv    - per-grid-point summary
  task4_failure_heatmap.png - DINO-C heatmap with failure boundary
"""
import json
import os
import sys
...
# Reuse the 5 pairs from Task 2 for comparability
SAMPLE_PAIRS = [
...
# Sweep grid (production point: lambda_LL=1.0, eta=0.1)
LAMBDA_LL_GRID = [0.0, 0.1, 0.3, 1.0, 3.0, 10.0]
ETA_GRID = [0.0, 0.1, 0.5, 1.0, 2.0]
DINO_C_FAILURE_THRESHOLD = 0.215  # TGT DINO-C floor
...
def compute_dino_c(src_img, gen_img, model, transform, device):
    """DINO-C: cosine similarity between gen CLS and source CLS (content preservation)."""
...
def main():
    print("=" * 70)
    print("Task 4: Failure Boundary Mapping (lambda_LL x eta grid)")
    print("=" * 70)
    ...
    # Save production cfg values
    mcfg = inf.model.model_cfg
    orig_adain = getattr(mcfg, "endpoint_adain_scale", 0.0)
    orig_adain_ll = getattr(mcfg, "endpoint_adain_scale_ll", -1.0)
    orig_extrap = getattr(mcfg, "style_extrap_alpha", 0.0)
    print(f"  Production: endpoint_adain_scale={orig_adain}, "
          f"endpoint_adain_scale_ll={orig_adain_ll}, style_extrap_alpha={orig_extrap}")

    # Load DINOv2 for DINO-C
    ...
    for src_style, src_file, tgt_style in SAMPLE_PAIRS:
        ...
        pair_data.append({
            "src_style": src_style, "src_file": src_file, "tgt_style": tgt_style,
            "src_path": str(src_path), "ref_path": str(ref_path),
    ...
    for lam_ll in LAMBDA_LL_GRID:
        for eta in ETA_GRID:
            ...
            print(f"\n[{point_idx}/{total_points}] lambda_LL={lam_ll}, eta={eta}")

            # Override cfg: keep endpoint_adain_scale=1.0 (production),
            # set endpoint_adain_scale_ll=lam_ll (overrides fallback),
            # set style_extrap_alpha=eta
            set_cfg(inf.model,
            ...
            for p in pair_data:
                ...
                try:
                    ...
                    # Check for NaN/Inf (gradient explosion signature)
                    if not torch.isfinite(z_out).all():
                        crashed = True
                        crash_msg = f"non-finite latent (NaN/Inf)"
                        pair_results.append({
                    ...
                    with torch.no_grad():
                        ...
                        # Check image is not all-black/all-white (another collapse signature)
            ...
            # Save intermediate results after each grid point (resume support)
            (OUTPUT_DIR / "task4_grid_results.json").write_text(
                json.dumps(grid_results, indent=2), encoding="utf-8")

    # Write CSV summary
    import csv
    csv_path = OUTPUT_DIR / "task4_grid_summary.csv"
    ...
    try:
        ...
        # Build DINO-C matrix (lambda_ll rows, eta cols)
        ...
        # Use a diverging colormap: red = failure, green = safe
        cmap = plt.cm.RdYlGn.copy()
        cmap.set_bad(color='#666666')  # crashed = gray
        im = ax.imshow(dino_c_matrix, cmap=cmap, vmin=0.0, vmax=0.6, aspect='auto',
                       origin='lower')
        ...
        # Mark production point (lambda_ll=1.0, eta=0.1)
        prod_i = LAMBDA_LL_GRID.index(1.0)
        prod_j = ETA_GRID.index(0.1)
        ax.plot(prod_j, prod_i, 'w*', markersize=20, markeredgecolor='black', markeredgewidth=1.5)
        ...
        ax.set_title("Failure Boundary: DINO-C heatmap\n(red dashed = TGT floor 0.215, * = production)", fontsize=11)
        plt.colorbar(im, ax=ax, label="DINO-C (content preservation)")
        plt.tight_layout()
        heatmap_path = OUTPUT_DIR / "task4_failure_heatmap.png"
