...
The method is not an exact Schr\"odinger Bridge solver---it does not
estimate forward and backward stochastic scores---but the three-part
design admits a bounded formal analysis (Sec.~\ref{sec:formal}):
...
\toprule
Method & Params & CLIP-S$\uparrow$ & LPIPS$\downarrow$ & EC$\uparrow$ & Infer (s) & ms/img & Train (s) \\
\midrule
...
\smallskip
\footnoterule\small
$\dagger$Extrapolated. ms/img = milliseconds per image for 750-output end-to-end inference.
\end{table*}
...
AdaIN remains faster at inference but lies clearly below the high-quality frontier (LPIPS 0.630, EC 0.264), while StyleID attains the highest raw CLIP-style (0.760) at the cost of severe content drift (LPIPS 0.750, CLIP-content 0.552) and much slower inference (603 s per 750 images). CAST also trails on core metrics. Additional baselines (StyTr2 48.3M, AesPA-Net 24.2M, AesFA 3.2M) are included in Table~\ref{tab:main} with measured or estimated FLOPs (603.2, 246.1, 25.3 GFLOPs respectively); their training times are unavailable from our protocol.
...
by path length) 0.98, while removing terminal SWD produces near-perfect
...
path regularity, while terminal SWD introduces controlled curvature
