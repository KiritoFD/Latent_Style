...
def train_efficiency_pareto():
    """EC vs training time (log scale). Marker size = params."""
    data = [
        ("Ours", 0.393, 310, 3.9, "#e64b35"),
        ("SaMST", 0.384, 6769, 6.0, "#000000"),
        ("S2WAT", 0.338, 10600, 65, "#00a087"),
        ("AdaIN", 0.264, 9220, 5, "#3c5488"),
        ("StyleID", 0.190, 0.1, 30, "#4dbbd5"),
        ("CAST", 0.182, 0.1, 7.0, "#8491b4"),
    ]
    fig, ax = plt.subplots(figsize=(4.8, 4.0))
    for name, ec, train_sec, params, c in data:
        tx = max(train_sec, 1)
        ax.scatter(tx, ec, s=60 + 500 * ec, c=c, edgecolor="white", linewidth=0.8, alpha=0.9, zorder=5)
        off = (0.15, 0.002) if name in ("Ours",) else (0.12, -0.004)
        ax.annotate(name, (tx, ec), (tx + off[0] * tx, ec + off[1]),
                    fontsize=8, ha='left', va='bottom',
    ...
    ax.set_title("Training-efficiency Pareto frontier")
...
def main():
    ...
    captions = {
        ...
        "fig_train_efficiency_pareto": "Training-efficiency Pareto plot (log scale). Ours achieves the highest EC among compared methods while operating in the lowest training-cost region."
    }
