...
class OTFlowMatchingObjective:
    ...
    def _compute_omf(
        ...
        *,
        ...
        if self.terminal_swd_weight > 0.0 and attn_plan is not None and semantic_k is not None:
            pseudo_target = self._barycentric_target(
                target_style,
                attn_plan,
                output_size=(pred_endpoint.shape[-2], pred_endpoint.shape[-1]),
            )
            _, pseudo_tgt_high = self._freq_split(pseudo_target, kernel_size=self.low_freq_kernel_size)
            swd_loss = self._semantic_guided_swd(pred_high, pseudo_tgt_high, semantic_k)
        else:
