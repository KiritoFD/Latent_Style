"""Generate Identity / AdaIN / WCT baseline images for wikiarts-20 (or any style list).
...
Skip logic: if the output PNG already exists, generation is skipped (resumable).

Usage (Windows remote):
    python gen_trainfree_wikiarts20.py --method identity --image-root I:\\datasets\\wikiarts20_512_test --output-root I:\\...\\exp\\baseline_wikiarts20
    python gen_trainfree_wikiarts20.py --method adain   ...
    python gen_trainfree_wikiarts20.py --method wct     ...
    python gen_trainfree_wikiarts20.py --method all     ...
"""
...
# wikiarts-20: full 20 styles (random-20 = 15 non-distinct5 + 5 distinct5)
DEFAULT_STYLES = [
...
def run_identity(sources, styles, out_dir: Path):
    ...
    print(f"  [identity] {n} new + {skipped} skipped in {elapsed:.1f}s", flush=True)
    return n
    ...
    with torch.inference_mode():
        for src_style, src_path in sources:
            ...
            for tgt_style, style_ref in style_refs.items():
                ...
                save_image(output.cpu().clamp(0, 1), out_path)
                n += 1
    elapsed = time.time() - t0
    rate = n / elapsed if elapsed > 0 else 0.0
    print(f"  [{method}] {n} new + {skipped} skipped in {elapsed:.1f}s ({rate:.1f} img/s)", flush=True)
    return n
...
def main():
    ...
    parser.add_argument("--styles", type=str, default=",".join(DEFAULT_STYLES),
                        help="Comma-separated style list (default: wikiarts-20 20 styles).")
    ...
    print(f"=== Train-free Baseline Inference (wikiarts-20) ===", flush=True)
