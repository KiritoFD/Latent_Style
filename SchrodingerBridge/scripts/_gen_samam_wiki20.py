#!/usr/bin/env python3
"""Generate SaMam baseline images for WikiArt-20 Distinct5 subset.

SaMam is image-based: content image + style image -> stylized output.
...
# Distinct5 styles (consistent with pipeline)
STYLES = [
...
def main():
    print(f"=== SaMam WikiArt-20 Distinct5 Inference ===", flush=True)
    ...
    pbar = tqdm(total=total, desc="samam_w20")
        ...
        for src_path in files:
            ...
            for tgt_style in STYLES:
                ...
                if (n_new + n_skip) % 50 == 0:
                    ...
                    print(f"  progress: {n_new + n_skip}/{total}  new={n_new} skip={n_skip}  rate={rate:.1f}/s  eta={eta:.0f}s", flush=True)
