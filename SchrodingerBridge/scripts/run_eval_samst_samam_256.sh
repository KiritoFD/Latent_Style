...
OUT="/mnt/i/exp_256_photo2art/eval_samst_samam_256.json"

cd "$REPO"
export PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True,max_split_size_mb:128
export OMP_NUM_THREADS=4

echo "[INFO] SAMST + SaMam 256 evaluation on photo2art"
echo "START=$(date '+%Y-%m-%dT%H:%M:%S')"

timeout 2400 "$PYTHON" /mnt/c/Users/Administrator/batch_compute_photo2art.py \
    --methods-json /mnt/c/Users/Administrator/methods_samst_samam_256.json \
    --output "$OUT" \
    --device cuda \
    --max-images 750 \
    --max-gen-artfid 200 \
