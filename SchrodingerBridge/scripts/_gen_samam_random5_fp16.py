"""Generate SaMam baseline for Random5 (5 styles, 750 pairs total, 619 remaining).

fp16, empty_cache every image, VRAM-safe.
Only generates the 5 Distinct5 styles (Early_Renaissance, Impressionism, Minimalism, Rococo, Ukiyo_e).
"""
import os
import sys
import time
...
IMAGE_SIZE = 256  # Reduced from 512 to avoid OOM on 12GB VRAM
STYLE_SIZE = 256  # SaMam is pixel-space, 256 still produces valid stylization
...
def load_model(ckpt_path, device):
    ...
    # Keep fp32 - model has internal mamba that doesn't support fp16 cleanly
    return model
...
def main():
    print(f"=== SaMam Random5 (fp16) ===", flush=True)
