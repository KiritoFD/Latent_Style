...
OUT="/mnt/i/exp_256_photo2art/eval_adain_256.json"

cd "$REPO"
export PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True,max_split_size_mb:128
export OMP_NUM_THREADS=4

echo "[INFO] AdaIN 256 evaluation on photo2art (legacy256_overfit50)"
echo "START=$(date '+%Y-%m-%dT%H:%M:%S')"

timeout 1800 "$PYTHON" /mnt/c/Users/Administrator/batch_compute_photo2art.py \
    --methods-json /mnt/c/Users/Administrator/methods_adain_256.json \
    --output "$OUT" \
    --device cuda \
    --max-images 750 \
    --max-gen-artfid 200 \
