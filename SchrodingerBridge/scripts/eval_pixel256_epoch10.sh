#!/usr/bin/env bash
# Run eval on pixel256 epoch_0010 ONLY, with image saving enabled.
...
CKPT=$REPO/exp/pixel256_photo2art/pixel256_b1_e5_softmax/epoch_0005.pt
OUTPUT=$REPO/exp/pixel256_photo2art/pixel256_b1_e5_softmax/full_eval/epoch_0005
LOG=/mnt/i/exp_256_photo2art/_eval_pixel256_epoch5.log

cd "$REPO"
export PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True,max_split_size_mb:128
export OMP_NUM_THREADS=4

echo "[INFO] Eval pixel256 epoch_0005 with image saving"
...
timeout 1200 "$PYTHON" -u "$REPO/src/utils/run_evaluation.py" \
    --checkpoint "$CKPT" \
    --output "$OUTPUT" \
