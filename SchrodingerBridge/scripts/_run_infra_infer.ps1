# Infra optimization: inference benchmark — multiple configs
# Usage: powershell -File _run_infra_infer.ps1
...
$logOut = "C:\Users\Administrator\logs\infra_infer_bench.out"
...
# Config 1: batch=4, save images (test larger batch with save)
Run-Eval "b4_save" @(
    "--batch_size", "4", "--generation_batch_size", "4", "--metric_batch_size", "4",
    "--target_chunk_size", "1", "--vae_decode_batch_size", "16"
)

# Config 2: batch=8, save images (push batch size)
Run-Eval "b8_save" @(
    "--batch_size", "8", "--generation_batch_size", "8", "--metric_batch_size", "8",
    "--target_chunk_size", "1", "--vae_decode_batch_size", "16"
)

# Config 3: batch=4, NO save images, NO summary grid (pure gen+metric time)
Run-Eval "b4_nosave" @(
    "--batch_size", "4", "--generation_batch_size", "4", "--metric_batch_size", "4",
    "--target_chunk_size", "1", "--vae_decode_batch_size", "16",
...
# Config 4: batch=8, NO save images, NO summary grid (fastest possible)
Run-Eval "b8_nosave" @(
    "--batch_size", "8", "--generation_batch_size", "8", "--metric_batch_size", "8",
    "--target_chunk_size", "1", "--vae_decode_batch_size", "16",
    "--no-save_generated_images", "--no-save_summary_grid"
...
Write-Output ""
Write-Output "=== ALL BENCHMARKS COMPLETE $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss') ==="
