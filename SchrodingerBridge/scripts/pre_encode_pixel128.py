#!/usr/bin/env python
r"""Pre-encode 128x128 pixel-space images as .pt tensors for the pixel-128 SFM ablation.

Walks a source directory of images (*.jpg / *.png), resizes each to 128x128 with LANCZOS,
converts to a float32 tensor of shape (3, 128, 128) normalized to [-1, 1], and saves as .pt
...
def encode_one(img_path: Path) -> torch.Tensor:
    """Load image, resize to 128x128 LANCZOS, return float32 tensor (3, 128, 128) in [-1, 1]."""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Pre-encode 128x128 pixel images as .pt tensors for pixel-128 SFM ablation"
    )
