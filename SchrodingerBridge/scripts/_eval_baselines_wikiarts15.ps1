# Evaluate Identity / AdaIN / WCT baselines on wikiarts-15 (15 styles, distinct5 excluded)
# Outputs to exp/baseline_wikiarts15/{method}/
#
# Pipeline per method:
#   1. Generate baseline images via scripts/gen_trainfree_wikiarts15.py
#      -> {OUT_ROOT}/{method}/images/*.png  +  {OUT_ROOT}/{method}/_DONE
#   2. Evaluate CLIP-S + LPIPS via src/utils/run_evaluation.py --reuse_generated
#      -> metrics.csv under {OUT_ROOT}/{method}/ (eval artifacts)
#
# Methods are invoked as separate Python processes for fault isolation and
# resumability via the _DONE marker (skips generation if already done).
...
$LOG = "$REPO\logs\baseline_wikiarts15.log"
...
"=== wikiarts-15 baselines eval started at $(Get-Date) ===" | Tee-Object -FilePath $LOG -Append
...
"--- Step 1: Generate baseline images ---" | Tee-Object -FilePath $LOG -Append
...
"=== wikiarts-15 baselines eval finished at $(Get-Date) ===" | Tee-Object -FilePath $LOG -Append
