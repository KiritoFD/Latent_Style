...
class OTFlowMatchingObjective:
    ...
    def _compute_omf(
        ...
        *,
        ...
        metrics["kinetic_energy"] = (kinetic_loss * self.w_kinetic).detach()
        ...
        if repel_raw is not None and self.w_repulsive > 0.0:
            total_loss = total_loss + repel_raw * self.w_repulsive
            metrics["repulsive"] = (repel_raw * self.w_repulsive).detach()
        else:
