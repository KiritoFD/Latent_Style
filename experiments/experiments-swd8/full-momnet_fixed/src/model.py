...
class LatentAdaCUT(nn.Module):
    """
    Micro U-Net with a single high-resolution skip fusion in the decoder.
    ...
    def __init__(
        ...
        self.skip_to_body = nn.Conv2d(self.lift_channels, self.body_channels, kernel_size=1, stride=1, padding=0)
    ...
    def _decode_core(
        ...
        if skip is not None:
            ...
            if skip.dtype != h.dtype:
                skip = skip.to(dtype=h.dtype)
            if skip.device != h.device:
                skip = skip.to(device=h.device)
            h = h + skip
        h = self.dec_conv(h)
        if self.use_decoder_adagn:
    ...
    def _run_decoder(
        ...
        if self.use_checkpointing and self.training:
            if torch.is_tensor(gate):
                ...
                gate_in = h.new_tensor(max(0.0, min(1.0, gate_val)))
            if skip is not None:
                skip_in = skip.to(device=h.device, dtype=h.dtype)
                return ckpt.checkpoint(
                    lambda _h, _k, _s, _g: self._decode_core(_h, _k, _s, _g),
                    h,
                    skip_in,
                    style_code,
                    gate_in,
            ...
            return ckpt.checkpoint(
                lambda _h, _s, _g: self._decode_core(_h, None, _s, _g),
                h,
                style_code,
                gate_in,
                use_reentrant=False,
            )
        return self._decode_core(h, skip, style_code, gate=gate)
    ...
    def _predict_delta_from_context(
        ...
        *,
        ...
        skip_hires = self.skip_to_body(h)
...
def build_model_from_config(
    ...
    *,
    ...
    return LatentAdaCUT(
        ...
        inject_gate_hires=float(model_cfg.get("inject_gate_hires", 1.0)),
