...
`StyleMaps` 是风格空间映射的容器模块。  
它本身不做复杂计算，但它在模型史里非常重要，因为它把“风格不是只有一个向量，而是可能在不同尺度上以 map 形式存在”这件事正式写进了结构。

## 为什么要这样写

项目很早就意识到一个问题：

- 大块色调、整体构图倾向、局部笔触、高频纹理，不一定应该由同一种表示来承载。

`StyleMaps` 的作用就是给这种多尺度风格表示留出位置。  
它是从“单一 style embedding”迈向“多尺度风格图”的关键过渡结构。

## 它解决的历史问题

如果没有 `StyleMaps`，风格信息只能压在一个全局向量里，结果往往是：

- 整体有风格方向，
- 但局部笔触、块面和频率结构表达不够灵活。

有了它之后，项目才能把中频、大块面、高频纹理这些概念逐步拆开研究，这也直接通向后面的 patch 频谱实验。
