...
`LatentAdaCUT` 是整条项目线里的主模型壳体。  
它把 encoder、body、attention、skip、decoder、style modulation 这些零件组织成一台可训练的 latent U-Net 风格迁移器。

换句话说，前面那些模块是在讨论“某个部件怎么工作”，而 `LatentAdaCUT` 讨论的是：

**这些部件最终是怎么拼成一台机器的。**

## 为什么要这样写

这个模型壳体之所以不断演化，是因为项目的核心问题一直不是“有没有一个好模块”，而是：

- 风格信号应该从哪里进入；
- skip 和 decoder 谁负责什么；
- attention 应该在什么位置出现；
- identity、color、SWD 这些训练目标该落在哪种结构上。

`LatentAdaCUT` 的意义就在于，它把这些结构判断固化成每一代模型的总装图。

## 它解决的历史问题

在不同历史阶段里，`LatentAdaCUT` 承担的重点不同：

- 早期是把 AdaGN 风格注入和 U-Net 训练跑通；
- 中期是容纳 StyleMaps、纹理字典、decoder 调制；
- 后期则开始吸收 cross-attn、skip 路由、语义 attention、gate 等机制。

所以它不是一个静态模型名，而是整个项目架构演化的主舞台。
