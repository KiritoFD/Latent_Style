# SemanticCrossAttn 模块文档

## 先看结论：这个模块在干什么

`SemanticCrossAttn` 做的是内容特征和风格特征之间的语义级交互。  
它不是单纯追求“更多 attention”，而是试图让模型在更高层面判断：哪些内容区域应该吸收哪些风格区域的信息。

## 为什么要这样写

因为单靠局部纹理调制并不能自动解决一个更难的问题：

- 风格不只是笔触和颜色；
- 它还涉及语义上“哪些部分应该被怎样改写”。

`SemanticCrossAttn` 的设计，说明项目已经不满足于只做低层纹理搬运，而是开始尝试把风格迁移推进到语义对齐层面。

## 它解决的历史问题

这个模块主要服务于两类研究问题：

1. 纯局部纹理调制是否会导致内容和风格脱节；
2. attention 是否能在语义层帮助模型更聪明地路由风格。

它的出现也解释了为什么后续会频繁出现这些变量：

- `semantic_attn_temperature`
- `attn_gate_mode`
- `pos_emb`

因为一旦进入语义 attention，稳定性和路由方式就都会变成一等公民。

## 基本信息
- **类名**: SemanticCrossAttn
- **行号**: 第 404 行
- **位置**: model.py
- **创建时间**: 待考古
