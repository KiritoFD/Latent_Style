# NormFreeModulation 模块文档

## 先看结论：这个模块在干什么

`NormFreeModulation` 是 decoder 端的一种无归一化风格调制器。  
它不先做 GroupNorm/LayerNorm，而是直接用 style 预测出的 `gamma/beta` 去调制特征。

## 为什么要这样写

它被引入，是因为项目在 decoder 端碰到了一个很实际的问题：

- 归一化虽然稳定，
- 但也可能把局部对比度和细粒度纹理洗得太平。

对于重建末端来说，这种“先洗平再上色”的流程不一定总是理想的。  
于是 `NormFreeModulation` 代表了一种新思路：

**在更靠近输出的位置，保留局部对比和细节，让风格直接作用在原始特征幅值上。**

## 它解决的历史问题

它主要服务于后期这类问题：

- decoder 端的风格是不是被归一化削弱了；
- 高频纹理能不能在不被洗平的情况下保留下来；
- 模块复杂度能不能下降，同时仍保留有效风格路径。

所以它不只是“去掉 norm”，而是在尝试把 decoder 末端从“稳定优先”往“纹理表达优先”再推一步。

## 基本信息
- **类名**: NormFreeModulation
- **行号**: 第 517 行
- **位置**: model.py
- **创建时间**: 待考古
