# SpatialSelfAttention 模块文档

## 先看结论：这个模块在干什么

`SpatialSelfAttention` 负责在空间位置之间建立关系，让模型不只看局部卷积邻域，而能让一个位置直接参考更远处的位置特征。

它在这条项目线里的意义，不是“跟风加 attention”，而是补卷积天生的局限：

- 卷积擅长局部纹理；
- 但对全局结构、长距离关联、跨区域一致性处理有限。
...
这说明项目在实现层面就已经把一个重要问题摆在台面上：

**到底应该做全局关系建模，还是做更省、更稳的局部窗口关系建模。**

这不是纯工程优化，而是模型哲学选择：

- 全局 attention 更强，但更重、更不稳；
- 窗口 attention 更保守，但可能更贴近局部纹理问题。

## 它解决的历史问题

中后期实验已经不满足于“局部纹理能不能出来”，而开始关心：

- 全图区域之间是否协调
- 大块面之间的风格关系是否一致
- 局部 patch 监督会不会让模型只会看近邻

`SpatialSelfAttention` 就是用来补这部分能力的。  
它也解释了为什么 4 月那条线会出现很多关于：
...
的讨论，因为这些变量都会直接改变空间关系建模的行为。

## 基本信息
- **类名**: SpatialSelfAttention
- **行号**: 第 273 行
- **位置**: model.py
- **创建时间**: 待考古
