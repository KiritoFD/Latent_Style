...
`AttentionBlock` 是一个组合块，它把两件本来可以分开做的事绑在了一起：

1. 先做 style-conditioned 的特征预调制；
2. 再做真正的空间自注意力。

所以它不是单纯的 self-attention 层，而是一个“先确定风格上下文，再做空间关系建模”的中间结构。

## 为什么要这样写

如果只做裸的 self-attention，模型学到的是内容位置之间的关系；  
如果只做风格调制，又容易缺少更大范围的空间组织能力。

`AttentionBlock` 的设计逻辑很像一个顺序假设：

- 先告诉当前特征“你应该处在什么风格语境里”；
- 再让它去和别的位置建立联系。

这说明项目并不把 attention 看成独立插件，而是把它看作风格语境下的空间关系推理器。

## 它解决的历史问题

这个模块主要服务于中后期这类需求：

- 模型不只要会局部调制，还要会更大范围地协调区域关系；
- 但这种协调不能脱离风格上下文单独发生。

这也是为什么后续 attention 一旦出问题，大家会同时去查：
