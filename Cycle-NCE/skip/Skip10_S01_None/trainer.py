...
class AdaCUTTrainer:
    ...
    def __init__(self, config: Dict, device: torch.device, config_path: Optional[str] = None) -> None:
        ...
        exp_dir = Path(self.checkpoint_dir)
        exp_dir.mkdir(parents=True, exist_ok=True)
        
        # Save config (already saved by run.py? but save again to be sure)
        ...
        logger.info(f"Experiment snapshot saved to {exp_dir}")
        # ==========================================
    ...
    def _apply_loss_hot_config(self, config: Dict) -> Dict[str, tuple[float | int | str | list[int], float | int | str | list[int]]]:
        ...
        if any(
            ...
            try:
                ...
                pass
            try:
                sobel_cache = getattr(lf, "_sobel_kernel_cache", None)
    ...
    def run_full_evaluation(self, epoch: int, checkpoint_path: Optional[Path] = None) -> bool:
        ...
        if bool(cfg_train.get("full_eval_enable_kid", False)):
            cmd += ["--eval_enable_kid"]
            cmd += ["--eval_kid_max_gen", str(int(cfg_train.get("full_eval_kid_max_gen", 200)))]
            cmd += ["--eval_kid_max_ref", str(int(cfg_train.get("full_eval_kid_max_ref", 200)))]
