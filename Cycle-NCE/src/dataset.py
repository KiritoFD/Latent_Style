...
class AdaCUTLatentDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> Dict[str, torch.Tensor]:
        ...
        if self.bidirectional_content:
            content_style_id = rng.randrange(len(self.style_subdirs))
            content_pool = self.style_tensors[content_style_id]
            content_idx = rng.randrange(content_pool.shape[0])
            content = content_pool[content_idx]

            target_candidates = [i for i in range(len(self.style_subdirs)) if i != content_style_id]
            target_style_id = target_candidates[rng.randrange(len(target_candidates))]
