...
set "TARGET_DIR=%ROOT_DIR%\45"
if not exist "%TARGET_DIR%" mkdir "%TARGET_DIR%"

for /d %%D in ("%ROOT_DIR%\45_*") do (
...
cd /d "%ROOT_DIR%"
uv run src/batch_distill_full_eval.py --exp_dir 45
if %errorlevel% neq 0 exit /b %errorlevel%

echo Distill/eval finished. Exporting CSV summary...
python import_summary_history_to_csv.py -i 45 -o 45.csv
if %errorlevel% neq 0 exit /b %errorlevel%

echo Running MA probe per experiment...
set "FINAL_EPOCH=0030"
for /d %%D in ("%TARGET_DIR%\45_*") do (
...
uv run src/probe_ma_sweep.py --input-glob "%TARGET_DIR%\45_*\ma_probe*.json" --output-dir "%TARGET_DIR%" --output-prefix ma_probe_all_pairs
if %errorlevel% neq 0 exit /b %errorlevel%
