...
def _load_base_config(src_dir: Path, base_config_arg: str | None) -> tuple[dict, Path]:
    if base_config_arg:
        ...
        base_path = next((p for p in candidates if p.exists()), candidates[0])
...
EXPERIMENTS: list[tuple[str, dict]] = [
    ...
    (
        "Z02_ResOff_Adapt_ZeroIDT_HFSWD",
        {
            "model": {
                "ablation_no_residual": True,
                "skip_routing_mode": "adaptive",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate ZeroConstraint experiment suite.")
    parser.add_argument(
        "--base-config",
        type=str,
        default=None,
