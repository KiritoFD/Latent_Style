...
echo [1/10] Running 00_holy_grail...
uv run run.py --config config_00_holy_grail.json
if %errorlevel% neq 0 exit /b %errorlevel%

echo [2/10] Running 01_highway_cut...
uv run run.py --config config_01_highway_cut.json
if %errorlevel% neq 0 exit /b %errorlevel%

echo [3/10] Running 02_dirty_skip...
uv run run.py --config config_02_dirty_skip.json
if %errorlevel% neq 0 exit /b %errorlevel%

echo [4/10] Running 03_decoder_usurpation...
uv run run.py --config config_03_decoder_usurpation.json
if %errorlevel% neq 0 exit /b %errorlevel%

echo [5/10] Running 04_muddy_routing...
uv run run.py --config config_04_muddy_routing.json
if %errorlevel% neq 0 exit /b %errorlevel%

echo [6/10] Running 05_micro_dictatorship...
uv run run.py --config config_05_micro_dictatorship.json
if %errorlevel% neq 0 exit /b %errorlevel%

echo [7/10] Running 06_hard_anchor...
uv run run.py --config config_06_hard_anchor.json
if %errorlevel% neq 0 exit /b %errorlevel%

echo [8/10] Running 07_rebuild_holy_grail...
uv run run.py --config config_07_rebuild_holy_grail.json
if %errorlevel% neq 0 exit /b %errorlevel%

echo [9/10] Running 08_natural_spectrum_reset...
uv run run.py --config config_08_natural_spectrum_reset.json
if %errorlevel% neq 0 exit /b %errorlevel%

echo [10/10] Running 09_real_holy_grail...
uv run run.py --config config_09_real_holy_grail.json
if %errorlevel% neq 0 exit /b %errorlevel%
...
uv run src/batch_distill_full_eval.py --exp_dir 46 --recursive --distill_mode tokenizer --distill_epochs 200 --batch_size 32
if %errorlevel% neq 0 exit /b %errorlevel%
