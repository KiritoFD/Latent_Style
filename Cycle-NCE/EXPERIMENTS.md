...
## 4.1 评估缓存排查（2026-02-09）
- 现象：多组实验 `classifier_acc` 与 `photo_to_art clip_style` 高度接近，怀疑缓存未清理。
- 排查结果：主要原因是模型落入同一失败局部最优，不是单纯缓存复用。
- 风险点修复：`src/utils/run_evaluation.py` 默认改为覆盖写 `metrics.csv`，避免同一输出目录追加历史行污染 summary。
- 新增参数：`--append_metrics`（仅在显式需要跨次累积时使用）。
