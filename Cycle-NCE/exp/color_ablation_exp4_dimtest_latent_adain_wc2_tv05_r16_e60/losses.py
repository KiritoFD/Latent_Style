...
def calc_latent_color_loss(
    ...
    *,
    ...
    del w_corr, w_gram, w_hist, w_moment3, w_moment4, w_spectrum, num_bins, hist_tau
    p = _lowfreq(pred, pool=pool, blur=blur)
    t = _lowfreq(target, pool=pool, blur=blur)
    mean_p = p.mean(dim=(2, 3))
    mean_t = t.mean(dim=(2, 3))
    std_p = torch.sqrt(p.var(dim=(2, 3), unbiased=False) + 1e-6)
    std_t = torch.sqrt(t.var(dim=(2, 3), unbiased=False) + 1e-6)
    ...
    if m in {"moments", "moment", "corr", "correlation", "stats_corr", "hist", "soft_hist", "cdf", "spectrum", "fft", "freq"}:
        return w_mean * mean_loss + w_std * std_loss + w_cov * cov_loss
    if m in {"stats_hist", "hist_cov", "stats_spectrum", "hist_spectrum"}:
    ...
    if m in {"full_distribution", "distribution", "color_full"}:
        return w_mean * mean_loss + w_std * std_loss + w_cov * cov_loss + w_wass * wass_loss + w_l1 * l1_loss
    return (
        w_mean * mean_loss
        + w_std * std_loss
        + w_cov * cov_loss
