# DecoderTextureBlock 模块文档

## 先看结论：这个模块在干什么

`DecoderTextureBlock` 是 decoder 端专门负责“把风格纹理真正写回到重建路径里”的模块。  
它的重点不在语义理解，而在最后一公里的纹理落地：让模型在上采样和重建后段，依然有一个明确的风格注入位置。

## 为什么要这样写

这样写是因为前面的实验已经反复说明：

1. 风格不是只在 bottleneck 里学到就结束了；
2. 许多笔触、颗粒、局部纹理，其实是在 decoder 末端才真正显现；
3. 如果 decoder 末端没有专门的风格块，前面学到的风格可能在重建过程中被冲淡。

所以这个模块本质上是在回答：

**decoder 到底只是恢复内容，还是也应该主动承担纹理生成职责。**

## 它解决的历史问题

这和 04-05 到 04-06 那条结构线是直接连着的：
...
`DecoderTextureBlock` 就是那条争论里最明确的工程答案之一。  
后续像 `decoder_usurpation` 这类实验，实际上就是在检验它到底应该多强。

## 基本信息
- **类名**: DecoderTextureBlock
- **行号**: 第 542 行
- **位置**: model.py
- **创建时间**: 待考古
