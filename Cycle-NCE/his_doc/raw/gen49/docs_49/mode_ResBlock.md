...
`ResBlock` 是这条模型线最基本的特征处理单元。  
它负责在不打断主干信息流的前提下，让风格调制、卷积变换和非线性更新可以一层层叠加上去。
...
1. 给内容结构留一条稳定的直通路径；
2. 允许模型逐步叠加风格改动，而不是每一层都从头重写整张特征图；
3. 在实验上提供一个很清楚的对照开关，便于验证“残差到底是不是生命线”。

这也是为什么后来很多实验会围绕 `ablation_no_residual`、`residual_gain` 展开。  
因为在这条项目线里，残差不是普通配件，而是 style/content 平衡的核心杠杆之一。

## 它解决的历史问题

没有残差时，模型更容易出现两种极端：

- 直接退化成 identity shortcut；
- 或者反过来为了追风格，把内容结构彻底冲坏。

`ResBlock` 提供的就是一个中间层次的可控更新机制，让风格改动有地方叠加，但不至于一下子破坏整条主干。
