...
def save_im():
    fig, ax = plt.subplots(figsize=(20, 10), facecolor=C["bg"])
    ax.set_xlim(0, 20)
    ax.set_ylim(0, 10)
    ax.axis("off")
    ax.set_aspect("equal")
    ...
    for (label, fill, stroke), y, h in zip(sec_labels, sec_y, sec_h):
        bg = FancyBboxPatch((0.25, y), 19.5, h,
    ...
    # --- Title ---------------------------------------------------------------
    ax.text(10, 9.85, "Spectral ODE Bridge", fontsize=28, weight="bold",
    ...
    ax.text(9.9, y2 + 1.05, "Shared Backbone (×4)", fontsize=11, weight="bold",
    ...
    # H_1
    h1 = FancyBboxPatch((15.4, y2 + 1.05), 1.0, 1.0,
                        boxstyle="round,pad=0.01,rounding_size=0.12",
                        facecolor="#ECFDF5", edgecolor=C["spectral_dark"], linewidth=2.2, zorder=2)
    ax.add_patch(h1)
    icon_grid(ax, 15.9, y2 + 1.55, 0.55, C["spectral"])
    ax.text(15.9, y2 + 1.12, "$\\hat{H}_1$", fontsize=11, weight="bold",
            color=C["spectral_dark"], ha="center", va="center")

    # iDWT
    idwt = FancyBboxPatch((16.7, y2 + 1.05), 1.0, 1.0,
                          boxstyle="round,pad=0.01,rounding_size=0.12",
                          facecolor=C["white"], edgecolor=C["content_dark"], linewidth=2.2, zorder=2)
    ax.add_patch(idwt)
    icon_dwt(ax, 17.2, y2 + 1.55, 0.5, C["content"])
    ax.text(17.2, y2 + 1.12, "iDWT", fontsize=10, weight="bold", ha="center", va="center")

    # Endpoint AdaIN/WCT
    ep = FancyBboxPatch((17.95, y2 + 1.05), 1.4, 1.0,
                        boxstyle="round,pad=0.01,rounding_size=0.12",
                        facecolor="#FEF3C7", edgecolor=C["style_dark"], linewidth=2.2, zorder=2)
    ax.add_patch(ep)
    ax.text(18.65, y2 + 1.55, "Endpoint", fontsize=10, weight="bold",
            color=C["style_dark"], ha="center", va="center")
    ax.text(18.65, y2 + 1.25, "AdaIN / WCT", fontsize=9,
            color=C["style_dark"], ha="center", va="center")

    # VAE Decode
    vae_dec = FancyBboxPatch((17.95, y2 - 0.1), 1.4, 0.9,
                             boxstyle="round,pad=0.01,rounding_size=0.12",
                             facecolor=C["white"], edgecolor=C["content_dark"], linewidth=2.2, zorder=2)
    ax.add_patch(vae_dec)
    icon_vae(ax, 18.65, y2 + 0.35, 0.6, C["content"])
    ax.text(18.65, y2 - 0.02, "VAE Decode", fontsize=9, weight="bold", ha="center", va="center")

    # Output image
    out = FancyBboxPatch((0.6, y2 - 0.1), 1.1, 1.1,
                         boxstyle="round,pad=0.01,rounding_size=0.1",
                         facecolor=C["white"], edgecolor=C["style_dark"], linewidth=2.2, zorder=2)
    ax.add_patch(out)
    embed_img(ax, 0.65, y2 - 0.05, img_w - 0.1, img_h - 0.1,
              OUT_DIR / "thumbs/output_thumb.png")
    ax.text(1.15, y2 - 0.22, "output $\\hat{x}$", fontsize=10, weight="bold", ha="center", va="top")
    ...
    # DWT -> LH/HL not shown separately, merged into H_t
    ...
    # ODE -> H1
    arrow([(15.1, y2 + 1.55), (15.4, y2 + 1.55)], color="black", lw=2)
    arrow([(16.4, y2 + 1.55), (16.7, y2 + 1.55)], color="black", lw=2)
    arrow([(18.7, y2 + 1.55), (18.65, y2 + 1.05)], color="black", lw=2)
    # endpoint -> vae decode
    ax.plot([18.65, 18.65], [y2 + 1.05, y2 + 0.8], color="black", lw=2, zorder=1)
    # decode -> output (wrap around bottom)
    arrow([(17.95, y2 + 0.35), (1.65, y2 + 0.35)], color="black", lw=2)
    arrow([(1.15, y2 + 0.35), (1.15, y2 - 0.1)], color="black", lw=2)

    # LL bypass -> iDWT / endpoint? In our model LL is rejoined at iDWT.
    # Draw LL -> iDWT dashed, passing under backbone region
    arrow([(6.3, y2 + 2.35), (6.3, y2 + 2.65), (17.2, y2 + 2.65), (17.2, y2 + 2.05)],
          color=C["content_dark"], lw=1.8, dashed=True)

    # --- 3. TRAINING ----------------------------------------------------------
    y3 = 0.35
    # x_t
    xt = block(0.6, y3, 1.4, 0.9, "$x_t = (1-t)z_0 + tz_{target}$", fill=C["white"],
               stroke=C["train_dark"], fontsize=9)
    # DWT
    dwt_t = FancyBboxPatch((2.3, y3), 1.2, 0.9,
                           boxstyle="round,pad=0.01,rounding_size=0.12",
                           facecolor=C["white"], edgecolor=C["train_dark"], linewidth=2.2, zorder=2)
    ax.add_patch(dwt_t)
    icon_dwt(ax, 2.9, y3 + 0.55, 0.45, C["train"])
    ax.text(2.9, y3 + 0.18, "DWT", fontsize=10, weight="bold", ha="center", va="center")

    # predict
    pred = block(3.9, y3, 1.6, 0.9, "Predict\n$v_{LH}, v_{HL}$", fill=C["white"],
                 stroke=C["train_dark"], fontsize=9)
    # target
    tgt = block(5.8, y3, 1.6, 0.9, "Target\n$\\Delta = \\text{DWT}(z_{target}-z_0)$",
                fill=C["white"], stroke=C["train_dark"], fontsize=9)
    # loss
    loss = block(7.7, y3, 3.0, 0.9,
                 "$\\mathcal{L} = \\omega_{LH}\\|v_{LH}-\\Delta_{LH}\\|_2^2 + \\omega_{HL}\\|v_{HL}-\\Delta_{HL}\\|_2^2$\n($\\omega_{LL}=0$)",
                 fill="#FEE2E2", stroke=C["train_dark"], fontsize=9)

    arrow([(2.0, y3 + 0.45), (2.3, y3 + 0.45)], color="black", lw=2)
    arrow([(3.5, y3 + 0.45), (3.9, y3 + 0.45)], color="black", lw=2)
    arrow([(5.5, y3 + 0.45), (5.8, y3 + 0.45)], color="black", lw=2)
    arrow([(7.4, y3 + 0.45), (7.7, y3 + 0.45)], color="black", lw=2)

    # feedback to backbone
    ax.plot([9.2, 9.2], [y3 + 0.9, y3 + 1.45], color=C["train_dark"], lw=2, ls="--", zorder=1)
    ax.plot([9.2, 9.9], [y3 + 1.45, y3 + 1.45], color=C["train_dark"], lw=2, ls="--", zorder=1)
    ax.annotate("", xy=(9.9, y2 + 0.85), xytext=(9.9, y3 + 1.45),
                arrowprops=dict(arrowstyle="-|>", color=C["train_dark"], lw=2, ls="--"), zorder=1)

    # bypassed mechanisms
    bypass = FancyBboxPatch((11.2, y3), 3.0, 0.9,
                            boxstyle="round,pad=0.01,rounding_size=0.12",
                            facecolor="#F3F4F6", edgecolor=C["gray_dark"], linewidth=2, zorder=2)
    ax.add_patch(bypass)
    ax.text(12.7, y3 + 0.75, "Bypassed (Ablated)", fontsize=10, weight="bold",
            color=C["gray_dark"], ha="center", va="center")
    ax.text(12.7, y3 + 0.38, "×  Euclidean OT Matching\n×  GroupNorm / Whitening\n×  Multi-step Style Guidance",
            fontsize=9, color=C["gray_dark"], ha="center", va="center")
    ...
    ax.text(10, -0.42,
            ...
            "rejoins at iDWT. Style is injected only at the endpoint via AdaIN/WCT ($c_s$), "
            "while style tokens condition the backbone cross-attention. Training supervises only LH/HL velocities.",
            fontsize=10, color=C["text"], ha="center", va="top", wrap=True)

    plt.tight_layout(pad=0.2)
    png = OUT_DIR / "aaai_arch_diagram_v7.png"
    svg = OUT_DIR / "aaai_arch_diagram_v7.svg"
