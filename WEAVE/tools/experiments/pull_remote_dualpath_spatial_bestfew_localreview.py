...
RUN_NAME = "aaai2027_inmortal_knee_e13_spatial_carriergate_bodydecoder_qedgegated_dualpath_spatial_seed42_b8a2"
...
DEFAULT_LOCAL_ROOT = SB_ROOT / "aaai2027" / "dualpath_spatial_bestfew_localreview_20260609"


def _run(cmd: list[str]) -> int:
    print("[pull_remote_dualpath_spatial_bestfew_localreview] " + " ".join(str(x) for x in cmd), flush=True)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Pull remote dualpath spatial eval curve and best-few epoch dirs to local storage.")
