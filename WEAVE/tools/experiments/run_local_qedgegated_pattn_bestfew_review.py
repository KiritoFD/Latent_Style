...
DEFAULT_AAAI_DIR = SB_ROOT / "aaai2027" / "qedgegated_pattn_bestfew_localreview_20260609"
...
def _run(cmd: list[str]) -> int:
    print("[run_local_qedgegated_pattn_bestfew_review] " + " ".join(str(x) for x in cmd), flush=True)
    env = os.environ.copy()
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Pull qedgegated+pattn best-few locally and run local IntroStyle/DINO review.")
    ...
    if not bool(args.skip_pull):
        pull = [
            sys.executable,
            str(SCRIPT_DIR / "pull_remote_qedgegated_pattn_bestfew_localreview.py"),
            "--eval-subdir",
            eval_subdir,
            "--local-root",
            str(aaai_dir),
    ...
    if not _images_present(handoff_csv):
        print(
            "[run_local_qedgegated_pattn_bestfew_review] images are absent in the pulled eval dirs; "
