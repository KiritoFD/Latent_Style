...
DEFAULT_AAAI_DIR = SB_ROOT / "aaai2027" / "dualpath_spatial_sinkhorn_bestfew_localreview_20260609"
...
def _run(cmd: list[str]) -> int:
    print("[run_local_dualpath_spatial_sinkhorn_bestfew_review] " + " ".join(str(x) for x in cmd), flush=True)
    env = os.environ.copy()
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run local IntroStyle/DINO review on pulled sinkhorn best-few eval dirs.")
    ...
    eval_subdir = str(args.eval_subdir).strip() or "full_eval"
    ...
    if not bool(args.skip_pull):
        pull = [
            sys.executable,
            str(SCRIPT_DIR / "pull_remote_dualpath_spatial_sinkhorn_bestfew_localreview.py"),
            "--eval-subdir",
            eval_subdir,
            "--local-root",
            str(aaai_dir),
    ...
    manifest = [
        ...
        "LBM",
        "--label-prefix",
        "DualPathSpatialSinkhornBestFew",
        "--source-root",
        str(SOURCE_ROOT),
    ]
    rc = _run(manifest)
    ...
    if not _images_present(handoff_csv):
        print(
            "[run_local_dualpath_spatial_sinkhorn_bestfew_review] images are absent in the pulled eval dirs; "
