...
python3 tools/experiments/launch_remote_wsl_command.py \
  --task-name phase618_plain_path_distill_wait_idle \
  --remote-workspace-root /mnt/i/Github/Latent_Style \
  --remote-wsl-cwd /mnt/i/Github/Latent_Style/SchrodingerBridge \
  --remote-log-path /mnt/i/Github/Latent_Style/SchrodingerBridge/docs/616/logs/phase618_plain_path_distill_wait_idle.log \
  --host 100.115.18.62 \
  --port 2222 \
  --user administrator \
