...
STAGE_ROOT="${STAGE_ROOT:-$ROOT/exp/20250618_plain_path_distill_auto}"
PLAIN_PATH_DISTILL_FIXED_BATCH_SIZE="${PLAIN_PATH_DISTILL_FIXED_BATCH_SIZE:-20}"
GENERATED_BASE_CFG="$STAGE_ROOT/_base_phase618_plain_path_distill_lowrank.json"
...
echo "============================================"
echo "  $(date) START phase618 plain-path distill"
echo "  base_cfg=$BASE_CFG"
echo "  stage_root=$STAGE_ROOT"
echo "  fixed_batch_size=$PLAIN_PATH_DISTILL_FIXED_BATCH_SIZE"
echo "============================================"
...
if issues:
    raise SystemExit(
        "phase618 plain-path distill base must already be the repaired lowrank carrier.\n- " + "\n- ".join(issues)
    )
...
python3 tools/experiments/phase616_auto.py plain-path-distill \
  --base-cfg "$GENERATED_BASE_CFG" \
  --stage-root "$STAGE_ROOT" \
  --skip-probe \
  --fixed-batch-size "$PLAIN_PATH_DISTILL_FIXED_BATCH_SIZE" \
  "$@"

echo "============================================"
echo "  $(date) DONE phase618 plain-path distill"
echo "============================================"
