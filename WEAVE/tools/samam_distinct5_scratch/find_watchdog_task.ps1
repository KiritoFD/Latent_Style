...
$可疑 = $tasks | Where-Object {
    $_.TaskName -match 'watchdog|628|p7|p8|destructive|samam' -or
    $_.'Task To Run' -match 'watchdog|628|p7|p8|destructive|samam|_628_'
}
$可疑 | Select-Object TaskName, Status, 'Next Run Time', 'Last Run Time', 'Last Result', 'Task To Run', 'Schedule Type' | Format-List

Write-Host ""
Write-Host "===== All READY scheduled tasks (filter to suspicious) ====="
$tasks | Where-Object { $_.Status -eq 'Ready' -and $_.'Task To Run' -match 'powershell|\.bat|\.ps1' } |
    Select-Object TaskName, 'Next Run Time', 'Task To Run' | Format-Table -AutoSize
