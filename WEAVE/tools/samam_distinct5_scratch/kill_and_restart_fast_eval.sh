...
# Kill old slow eval
tmux kill-session -t samam_hf_eval 2>/dev/null || true
pkill -f "eval_samam_checkpoint_curve" 2>/dev/null || true
sleep 2

# Copy new scripts to I drive
...
echo "=== NEW eval launched (GPU-batched, batch_size=32) ==="
tmux ls
echo ""
