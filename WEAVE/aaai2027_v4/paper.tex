...
\begin{abstract}
Style transfer faces a practical and evaluative tension. Diffusion-based editors can be
costly to deploy, while compact alternatives may preserve the source so strongly that they
approach an identity mapping. Content-sensitive metrics can obscure this failure when the
unchanged source is not reported as an explicit reference.

We trace this identity shortcut to frequency dominance in a shared representation: strong
low-frequency structural residuals suppress weaker high-frequency style gradients. WEAVE
addresses this imbalance with wavelet-decoupled flow matching, separating structural and
stylistic transport while confining statistical correction to the endpoint. This division
reduces competition between content preservation and style acquisition without requiring a
large trainable backbone.

We further introduce the IDT--TGT evaluation guideline, which brackets transfer between the
unchanged source and a source-independent target-style image. Reporting these references
alongside standard metrics exposes both insufficient stylization and excessive content loss
without collapsing them into a composite score.

Across three benchmarks, WEAVE moves beyond the identity reference while maintaining source
content. It uses 1.04M trainable parameters with a frozen 84M-parameter VAE and trains in
1.4 minutes on one RTX~3060, providing a compact operating point for multi-style transfer.
...
StyTR-2 & 0.501 & 0.709 & 0.596 & 0.747 & 0.515 & 0.722 & 0.495 & 0.555 & 0.533 & 0.759 & 0.553 & 0.737 & 35.4$^{+12.9}$ & $\sim$1440$^{\ast}$ & 4\,m \\
AesPA-Net & 0.520 & 0.724 & 0.548 & 0.601 & 0.498 & 0.744 & 0.571 & 0.522 & 0.545 & 0.782 & 0.409 & 0.728 & 11.3$^{+20.0}$ & -- & 3\,m \\
...
\midrule
\textbf{Ours} & 0.4915 & 0.7126 & 0.2596 & 0.8103 & 0.4801 & 0.6681 & 0.3116 & \underline{\textbf{0.8612}} & 0.5226 & 0.7747 & \underline{\textbf{0.2895}} & 0.7717 & \underline{\underline{\textbf{1.04$^{+84}$}}} & \underline{\underline{\textbf{1.4}}} & \underline{\textbf{106\,s}} \\
\bottomrule
\end{tabular}%
}
\caption{Main comparison. DINO-S, CLIP-S, and DINO-C are higher better; LPIPS is lower better. Excluding IDT/TGT, the top two are bold: first has a double underline and second a single underline. Params/Train rank trained methods; Infer ranks reported runtimes. $\dagger$: style does not exceed IDT. $\ddagger$: content does not remain inside TGT. D5 uses oriented HF and internal selection; P2A/R5 use base checkpoints. Params are in millions; superscripts are frozen parameters. Other times use one RTX~3060; $\ast$ is the StyTR-2 author report (about one day on two P100 and two RTX~3090 GPUs~\cite{deng2022stytr2}).}
\label{tab:main}
\end{table*}
