...
\midrule
\textbf{Ours} & \textbf{0.4915} & 0.7126 & 0.2596 & 0.8103 & 0.4801 & 0.6681 & 0.3116 & 0.8612 & \underline{0.5226} & 0.7747 & 0.2895 & 0.7717 & \textbf{1.04M$^{+84M}$} & \textbf{1.4} & \textbf{106\,s} \\
\bottomrule
\end{tabular}%
}
\caption{Main comparison. DINO-S, CLIP-S, and DINO-C are higher better; LPIPS is lower better. IDT and TGT are empirical references, not theoretical bounds. $\dagger$: style value does not exceed IDT. $\ddagger$: content value does not remain inside TGT. The D5 row uses the oriented-HF route and internal-dynamics epoch selection; P2A and R5 report the base WEAVE checkpoints. Params, Train, and Infer describe the D5 model. Superscripts denote frozen parameters; Train is wall time on one RTX~3060 and Infer is generation-only time for 750 images.}
\label{tab:main}
\end{table*}
