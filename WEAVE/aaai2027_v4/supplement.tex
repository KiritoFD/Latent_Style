...
LL/HF ratio crosses below one. A retrospective probe of the complete 15-epoch curve selects
...
\caption{Training audit for the internally selected D5-512 checkpoint. Training stops at
epoch 4 when the target-HF gates start expanding and the shared LL/HF gradient ratio first
crosses below one.}
...
inference in the wavelet domain. The main D5-512 row uses
...
inference-time endpoint AdaIN strength. They remain diagnostic operating points and are
...
\bottomrule
\end{tabularx}
\caption{Inference-only operating points produced from the previous base checkpoint.}
...
(1.000, 0.874, 0.733, 0.176). TGT reaches \dinos{} 1.000 because the selected reference is
...
\textbf{WEAVE} & \textbf{0.492} & 0.713 & 0.260 & 0.810 & 0.480 & 0.668 & 0.312 & 0.861 & 0.523 & 0.775 & 0.290 & 0.772 & 1.04M$^{+84M}$ & 1.4 & 106 s \\
\bottomrule
\end{tabular}}
\end{table*}
