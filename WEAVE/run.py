...
def _run_full_eval_for_checkpoint(config: ExperimentConfig, checkpoint_path: Path) -> dict[str, object] | None:
    ...
    collector = project_root / "tools" / "experiments" / "collect_round2_eval_curve.py"
    convergence = project_root / "tools" / "experiments" / "report_round2_convergence.py"
    manifest = project_root / "tools" / "experiments" / "update_round2_family_manifest.py"
    gap_report = project_root / "tools" / "experiments" / "report_round2_reference_gap.py"
    ...
    try:
        ...
        manifest_csv = project_root / "docs" / "experiments" / "round2_pure_sde" / "round2_family_manifest.csv"
        ...
        if manifest.is_file() and manifest_csv.is_file() and family_name.startswith("aaai2027_round2_") and family_name.endswith("_seed42_b8a2"):
            ...
            if gap_spec is not None and gap_report.is_file() and curve_summary_json.is_file():
                gap_cmd = [
                    ...
                    "--json-out",
                    str(project_root / str(gap_spec["json_out_rel"])),
                ]
