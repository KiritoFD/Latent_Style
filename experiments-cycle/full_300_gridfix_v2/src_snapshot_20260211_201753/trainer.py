...
class AdaCUTTrainer:
    def __init__(self, config: Dict, device: torch.device, config_path: Optional[str] = None) -> None:
        ...
        self.snapshot_source = bool(train_cfg.get("snapshot_source", True))
    ...
    def train_epoch(self, dataloader: DataLoader, epoch: int) -> Dict[str, float]:
        ...
        # Keep running sums on-device as detached scalars; sync only at logging/finalization.
        ...
        for step_idx, raw_batch in enumerate(progress, start=1):
            try:
                ...
                # Accumulate detached scalar metrics on device (no graph retention).
                for k, v in loss_dict.items():
                    if v is None:
                        continue
                    metric_val = v.detach().float()
                    if k not in metric_accum:
                        metric_accum[k] = metric_val
                    else:
                        metric_accum[k] = metric_accum[k] + metric_val
                num_batches += 1
                ...
                if step_idx % self.log_interval == 0:
                    ...
                    if not self.use_tqdm:
                        logger.info(
                            "epoch %d step %d/%d | loss=%.4f sgram=%.4f cmoment=%.4f code=%.4f cpn=%.3f crn=%.3f cycle=%.4f semi=%.4f nce=%.4f | %.2f it/s eta %.1fs",
                            epoch,
                            step_idx,
                            total_steps,
                            _get_avg('loss'),
        ...
        expected_keys = [
            ...
            "push", "delta_tv", "style_spatial_tv", "nce", "semigroup", "w_cycle_eff", "w_struct_eff",
