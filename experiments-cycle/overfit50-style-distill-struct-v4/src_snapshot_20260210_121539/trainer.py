    ...
    def train_epoch(self, dataloader: DataLoader, epoch: int) -> Dict[str, float]:
        ...
        for step_idx, raw_batch in enumerate(progress, start=1):
            try:
                ...
                sum_edge += float(loss_dict.get("edge", torch.tensor(0.0, device=content.device)).item())
                ...
                sum_w_edge_eff += float(loss_dict.get("w_edge_eff", torch.tensor(0.0, device=content.device)).item())
        ...
        metrics = {
            ...
            "edge": sum_edge / max(num_batches, 1),
            ...
            "w_edge_eff": sum_w_edge_eff / max(num_batches, 1),
