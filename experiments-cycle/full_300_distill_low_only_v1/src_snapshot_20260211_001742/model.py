...
class LatentAdaCUT(nn.Module):
    ...
    def __init__(
        ...
        # Decoder: 16 -> 32
        self.dec_up = nn.Upsample(scale_factor=2, mode=self.upsample_mode, align_corners=False if self.upsample_mode in {"bilinear", "bicubic"} else None)
    ...
    def forward(
        ...
        if style_spatial_32 is not None and self.use_style_spatial_blur:
            style_spatial_32 = self._blur2d(style_spatial_32)
        ...
        if style_spatial_16 is not None and self.use_style_spatial_blur:
            style_spatial_16 = self._blur2d(style_spatial_16)
        ...
        if style_spatial_dec is not None and self.use_style_spatial_blur:
            style_spatial_dec = self._blur2d(style_spatial_dec)
