...
parser.add_argument('--img_size', type=int, default=256, help='Model anchor image size')
parser.add_argument('--train_size', type=int, default=0, help='On-the-fly resize for training inputs. <=0 means keep native resolution')
parser.add_argument('--precision', type=str, default='bf16', choices=['fp32', 'amp', 'bf16'], help='Training precision mode')
parser.add_argument('--grad_checkpoint', action='store_true', help='Enable gradient checkpointing in encoder')
...
if args.train_size > 0:
  train_tf = T.Compose([
    T.Resize((args.train_size, args.train_size)),
    T.ToTensor()
  ])
else:
  train_tf = T.ToTensor()
dataset_content = SimpleDataset(args.content_dir, transforms=train_tf)
dataset_style = SimpleDataset(args.style_dir, transforms=train_tf)
...
scaler = torch.amp.GradScaler('cuda', enabled=(torch.cuda.is_available() and args.precision == 'amp'))
...
if __name__ == '__main__':
  for i in range(args.epoch):
    ...
    # calculate losses
    if args.precision in ('amp', 'bf16'):
      amp_dtype = torch.float16 if args.precision == 'amp' else torch.bfloat16
