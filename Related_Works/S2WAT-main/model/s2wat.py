  ...
  def forward(self, x):
    for blk in self.blocks:
      if self.use_checkpoint:
        # torch.utils.checkpoint only tracks Tensor inputs reliably.
        # Keep metadata in closure and checkpoint the feature tensor path.
        x_tensor, arbitrary_input, input_resolution = x
        def _blk_forward(t):
          return blk((t, arbitrary_input, input_resolution))[0]
        x_tensor = checkpoint.checkpoint(_blk_forward, x_tensor, use_reentrant=False)
        x = (x_tensor, arbitrary_input, input_resolution)
      else:
        x = blk(x)
