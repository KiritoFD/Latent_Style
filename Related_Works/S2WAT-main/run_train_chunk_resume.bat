...
set "CKPT_DIR=%ROOT%\pre_trained_models\checkpoint_bs1_safe"
...
for /L %%R in (1,1,%ROUNDS%) do (
  ...
  for /f "usebackq delims=" %%I in (`powershell -NoProfile -Command ^
    "$d='%CKPT_DIR%'; if(Test-Path $d){$f=Get-ChildItem $d -File -Filter 'checkpoint_*_epoch.pkl' | Sort-Object { [int](($_.BaseName -replace 'checkpoint_','') -replace '_epoch','') } -Descending | Select-Object -First 1; if($f){$f.FullName}}"`) do (
    set "LATEST_CKPT=%%I"
  )
