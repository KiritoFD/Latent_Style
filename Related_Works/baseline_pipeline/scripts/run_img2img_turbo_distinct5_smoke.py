...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--pretrained-model-name-or-path", default="/mnt/i/Github/Latent_Style/Related_Works/runs/hf_snapshots/sd_turbo_snapshot_20260606")
    parser.add_argument("--train-img-prep", default="resize_512")
    parser.add_argument("--val-img-prep", default="resize_512")
    parser.add_argument("--train-batch-size", type=int, default=1)
    parser.add_argument("--lora-rank-unet", type=int, default=2)
    parser.add_argument("--lora-rank-vae", type=int, default=1)
...
def build_command(args: argparse.Namespace, dataset_dir: Path, output_dir: Path) -> list[str]:
    ...
    if args.skip_initial_validation:
        cmd.extend(["--validation_steps", str(max(int(args.validation_steps), int(args.max_train_steps) + 1))])
    return cmd
