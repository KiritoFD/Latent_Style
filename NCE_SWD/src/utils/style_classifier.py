...
class StyleClassifier(nn.Module):
    """
    Content-suppressed latent style classifier.

    Compatible with trainer/run_evaluation constructor signature:
    - use_stats / use_gram / use_lowpass_stats
    - spatial_shuffle
    - input_size_train / input_size_infer / lowpass_size
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        stats_dim = 0
        if self.use_stats:
            # highpass mean/std (texture-focused statistics)
            stats_dim += in_channels * 2
        if self.use_lowpass_stats:
            # lowpass mean/std (palette / coarse tone)
            stats_dim += in_channels * 2
        if self.use_gram:
            stats_dim += in_channels * in_channels

        head_in = w * 2 + stats_dim
    ...
    def _stats_pack(self, x: torch.Tensor) -> torch.Tensor:
        # x: [B,C,H,W]
        parts = []
        lp = _lowpass(x, lowpass_size=self.lowpass_size)
        hp = x - lp

        if self.use_stats:
            parts.append(hp.mean(dim=(2, 3)))
            parts.append(hp.std(dim=(2, 3), unbiased=False))

        if self.use_lowpass_stats:
            parts.append(lp.mean(dim=(2, 3)))
            parts.append(lp.std(dim=(2, 3), unbiased=False))

        if self.use_gram:
            B, C, H, W = hp.shape
            xf = hp.reshape(B, C, -1)
            gram = torch.bmm(xf, xf.transpose(1, 2)) / (H * W)
            parts.append(gram.reshape(B, -1))
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.training and self.spatial_shuffle:
            # stochastic layout disruption to reduce content shortcut reliance
            x = _patch_shuffle(x, grid=2)

        feat = self.backbone(x)
        stats = self._stats_pack(x)
        if stats.shape[1] > 0:
